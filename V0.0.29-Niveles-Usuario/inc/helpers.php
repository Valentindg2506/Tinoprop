<?php
/*
 * Archivo: inc/helpers.php — V0.0.29
...
function puede_acceder_seccion(string $seccion): bool
{
    ...
    if (es_superadmin()) {
        $secciones_admin = ['admin-dashboard', 'admin-inmobiliarias', 'admin-usuarios', 'peticiones', 'configuracion'];
        return in_array($seccion, $secciones_admin, true);
    }

    // Secciones universales (todos los roles)
    $universales = ['dashboard', 'recordatorios', 'matching', 'documentacion', 'configuracion', 'busqueda-avanzada', 'post-venta'];
 ...
 * SuperAdmin devuelve array vacío.
 */
...
function usuarios_asegurar_tabla(PDO $pdo): void
{
    ...
    $columnas = ['inmobiliaria_id', 'avatar_url', 'activo', 'password_temporal'];
...
function importar_csv_clientes(PDO $pdo, string $filepath, string $tipo): array
{
    ...
    $linea = 1;
    $stmt = $pdo->prepare(
        'INSERT INTO clientes (tipo, nombre, apellido, telefono, email, operacion, direccion, zona_interesada, presupuesto, comentarios, inmobiliaria_id)
         VALUES (:tipo, :nombre, :apellido, :telefono, :email, :operacion, :direccion, :zona, :presupuesto, :comentarios, :iid)'
    );
...
function importar_csv_propiedades(PDO $pdo, string $filepath, string $equipo): array
{
    ...
    $linea = 1;
    $stmt = $pdo->prepare(
        'INSERT INTO propiedades (equipo, titulo, tipo, ubicacion, direccion, metros, habitaciones, banos, precio, moneda, operacion, estado, referencia, descripcion, inmobiliaria_id)
         VALUES (:equipo, :titulo, :tipo, :ubicacion, :direccion, :metros, :hab, :banos, :precio, :moneda, :operacion, :estado, :ref, :desc, :iid)'
    );
