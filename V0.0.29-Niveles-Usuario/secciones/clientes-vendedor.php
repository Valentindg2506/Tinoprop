...
$stmt_total->execute(['tipo' => 'vendedor'] + sql_iid_params());
$total_clientes = (int)$stmt_total->fetchColumn();
$pagina_actual = max(1, (int)($_GET['pagina'] ?? 1));
$paginacion = paginar($total_clientes, 15, $pagina_actual);

$stmt = $pdo->prepare('SELECT id, nombre, apellido, telefono, email, operacion FROM clientes WHERE tipo = :tipo' . sql_iid() . ' ORDER BY id DESC LIMIT :lim OFFSET :off');
