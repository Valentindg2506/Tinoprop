...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['crear_prospecto'])) {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO prospectos (tipo, nombre, interes, estado, telefono, inmobiliaria_id)
             VALUES (:tipo, :nombre, :interes, :estado, :telefono, :iid)'
        );
        $stmt->execute([
            'tipo' => 'comprador',
            'nombre' => $nombre,
    ...
    if (isset($_POST['convertir_a_cliente'])) {
        ...
        if ($id_conv > 0) {
            ...
            if ($p) {
                ...
                $apellido_c = $partes[1] ?? '';
                $stmt2 = $pdo->prepare(
                    'INSERT INTO clientes (tipo, nombre, apellido, telefono, email, operacion, inmobiliaria_id)
                     VALUES (:tipo, :nombre, :apellido, :telefono, :email, :operacion, :iid)'
                );
                $stmt2->execute([
                    'tipo' => 'comprador',
                    'nombre' => $nombre_c,
...
$stmt = $pdo->prepare('SELECT id, nombre, interes, estado, telefono, created_at FROM prospectos WHERE tipo = :tipo' . sql_iid() . ' ORDER BY id DESC');
