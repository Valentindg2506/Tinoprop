...
*/
/*
 * Sección: Propiedades comprador
 * Rol: alta, listado y baja de propiedades de venta para el equipo comprador.
...
// Controlador POST del módulo: alta y baja de propiedades en venta del equipo comprador.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['crear_propiedad'])) {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO propiedades (equipo, titulo, tipo, ubicacion, direccion, metros, habitaciones, banos, precio, moneda, periodo, operacion, estado, referencia, descripcion, inmobiliaria_id)
             VALUES (:equipo, :titulo, :tipo, :ubicacion, :direccion, :metros, :habitaciones, :banos, :precio, :moneda, :periodo, :operacion, :estado, :referencia, :descripcion, :iid)'
        );
        $stmt->execute([
            'equipo' => 'comprador',
            'titulo' => $titulo,
...
// Listado de propiedades en venta para render de tarjetas.
$stmt = $pdo->prepare('SELECT id, titulo, tipo, ubicacion, metros, habitaciones, banos, precio, moneda, periodo, estado FROM propiedades WHERE equipo = :equipo AND operacion = :operacion' . sql_iid() . ' ORDER BY id DESC');
$stmt->execute([
    'equipo' => 'comprador',
    'operacion' => 'venta'
] + sql_iid_params());
