...
try {
    ...
    switch ($action) {
        case 'crear':
            ...
            if ($id) {
                // Sincronización: si el tipo es "Visita", crear entrada automática en tabla visitas.
        ...
        case 'eliminar':
            ...
            // Sincronización: eliminar visita vinculada si existe (sin eliminar el recordatorio de nuevo).
            visitas_asegurar_tabla($pdo);
            $usuario_id = (int) ($_SESSION['usuario']['id'] ?? 0);
            $stmtV = $pdo->prepare('SELECT id FROM visitas WHERE recordatorio_id = :rid AND usuario_id = :uid');
            $stmtV->execute(['rid' => $id, 'uid' => $usuario_id]);
