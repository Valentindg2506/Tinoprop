...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'cambiar_password') {
        ...
        if (strlen($nueva) < 8) {
            $errores[] = 'La nueva password debe tener al menos 8 caracteres.';
        }
...
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: 18px; align-items: start;">
    <section id="perfil" class="form_panel visible">
        <h3>Perfil</h3>
        <p>Estos datos identifican tu cuenta y se usan en avisos y actividad.</p>
        <ul style="margin: 10px 0 16px 18px; color: #34495e; line-height: 1.4;">
            <li>Usuario: <?php echo e($usuario['nombre']); ?> (Rol: <?php echo e($usuario['rol']); ?>)</li>
            <li>Email acceso: <?php echo e($usuario['email']); ?></li>
        </ul>
        <form method="POST" class="form_grid" style="grid-template-columns: repeat(2, minmax(0, 1fr));">
    ...
    <section id="seguridad" class="form_panel visible">
        <h3>Seguridad</h3>
        <p>Cambia tu password para mantener la cuenta protegida.</p>
        <form method="POST" class="form_grid" style="grid-template-columns: repeat(1, minmax(0, 1fr));">
            ...
            <div class="campo_input">
                <label for="password_nueva">Password nueva (min. 8)</label>
                <input id="password_nueva" name="password_nueva" type="password" required>
            </div>
            <div class="campo_input">
                <label for="password_repetir">Repetir password nueva</label>
                <input id="password_repetir" name="password_repetir" type="password" required>
            </div>
            <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                <button class="btn_guardar" type="submit">Actualizar password</button>
            </div>
        </form>
    </section>

    <section id="preferencias" class="form_panel visible">
        <h3>Preferencias del dashboard</h3>
        <p>Define con que filtros se abre el dashboard cuando ingresas sin seleccionar opciones.</p>
        <form method="POST" class="form_grid" style="grid-template-columns: repeat(2, minmax(0, 1fr));">
        ...
        <p style="margin-top: 10px; color: #7f8c8d;">Estos valores se aplican al abrir el dashboard si no hay filtros en la URL.</p>
    </section>
</div>
