...
$origen_raw = $_GET['origen'] ?? 'clientes-vendedor';
$origenes_validos = ['clientes-vendedor', 'clientes-comprador', 'prospectos-vendedor', 'prospectos-comprador', 'busqueda-avanzada', 'matching'];
$origen = in_array($origen_raw, $origenes_validos, true) ? $origen_raw : 'clientes-vendedor';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $id_cliente > 0) {
    ...
    if (isset($_POST['guardar_cambios'])) {
        ...
        if (!validar_requerido($operacion)) {
            $errores[] = 'La operacion es obligatoria.';
        }
        if (!empty($errores)) {
    ...
    if (isset($_POST['editar_nota'])) {
        ...
        if ($nid > 0 && $ntxt !== '') {
            nota_actualizar($pdo, $nid, $ntxt, $ntip);
            flash_set('success', 'Nota actualizada.');
        }
    }
    if (isset($_POST['eliminar_nota'])) {
        $nid = (int) ($_POST['nota_id'] ?? 0);
        if ($nid > 0) {
            nota_eliminar($pdo, $nid);
            flash_set('success', 'Nota eliminada.');
        }
    }
    if (isset($_POST['asignar_etiqueta'])) {
        $tid = (int) ($_POST['etiqueta_id'] ?? 0);
        if ($tid > 0) {
            entidad_asignar_etiqueta($pdo, 'cliente', $id_cliente, $tid);
        }
    }
    if (isset($_POST['quitar_etiqueta'])) {
        $tid = (int) ($_POST['etiqueta_id'] ?? 0);
        if ($tid > 0) {
            entidad_quitar_etiqueta($pdo, 'cliente', $id_cliente, $tid);
        }
    }
    if (isset($_POST['crear_etiqueta_rapida'])) {
        ...
        if ($tn) {
            $new_id = etiqueta_crear($pdo, $tn, $tc);
            if ($new_id) {
                entidad_asignar_etiqueta($pdo, 'cliente', $id_cliente, $new_id);
            }
        }
    }
    if (isset($_POST['eliminar_cliente'])) {
        try {
            ...
            $pdo->prepare("DELETE FROM entidad_etiquetas WHERE entidad_tipo = 'cliente' AND entidad_id = :id" . sql_iid())->execute(['id' => $id_cliente] + sql_iid_params());
...
<div class="barra_tags animate_fadeIn">
    ...
    <details class="tag_agregar_detail">
        ...
        <div class="tag_dropdown">
            <?php $tags_ids = array_column($tags_cliente, 'id');
    $tags_disp = array_filter($todas_tags, fn ($t) => !in_array($t['id'], $tags_ids)); ?>
                ...
                <ul class="lista_notas">
                    ...
                    <li class="nota_item" id="nota-<?php echo $nota['id']; ?>">
                        <div class="nota_meta">
                            ...
                            <button type="button" class="btn_icono_sm" onclick="editarNota(<?php echo (int) $nota['id']; ?>)" title="Editar">✏️</button>
                        </div>
                        ...
                        <div class="nota_edit_form" id="nota-edit-<?php echo $nota['id']; ?>" style="display:none">
                            ...
                            <div class="nota_edit_btns">
                                ...
                                <button type="button" class="btn_secundario btn_chico" onclick="cancelarEditNota(<?php echo (int) $nota['id']; ?>)">Cancelar</button>
                            </div>
                        </div>
                    </li>
                    <?php endforeach; ?>
            ...
            <div class="panel_timeline">
                ...
                <ul class="timeline_lista">
                    ...
                    <li class="timeline_item timeline_item--<?php echo e($ev['origen']); ?>">
                        <span class="timeline_icono"><?php echo match($ev['origen']) {
                            'nota' => '📝','actividad' => '⚡','visita' => '🚶','oferta' => '💰',default => '📋'
                        }; ?></span>
