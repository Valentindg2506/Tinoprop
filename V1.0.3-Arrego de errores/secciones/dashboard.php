...
if ($desde_anterior) {
    ...
    $st->execute($p_ant2);
    $prev_venta = (int) $st->fetchColumn();
    $p_ant2['operacion'] = 'alquiler';
    $st->execute($p_ant2);
    $prev_alquiler = (int) $st->fetchColumn();
}
...
if ($equipo_db) {
    $sql_vis_chart .= ' AND equipo = :equipo';
    $p_vis['equipo'] = $equipo_db;
}
if ($desde) {
    $sql_vis_chart .= ' AND fecha_visita >= :desde';
    $p_vis['desde'] = $desde;
}
...
if ($equipo_db) {
    $sql_of_chart .= ' AND p.equipo = :equipo';
    $p_of['equipo'] = $equipo_db;
}
if ($desde) {
    $sql_of_chart .= ' AND o.fecha_oferta >= :desde';
    $p_of['desde'] = $desde;
}
...
if ($equipo_db) {
    $sql_prop_chart .= ' AND equipo = :equipo';
    $p_prop['equipo'] = $equipo_db;
}
if ($operacion_db) {
    $sql_prop_chart .= ' AND operacion = :operacion';
    $p_prop['operacion'] = $operacion_db;
}
if ($desde) {
    $sql_prop_chart .= ' AND created_at >= :desde';
    $p_prop['desde'] = $desde;
}
