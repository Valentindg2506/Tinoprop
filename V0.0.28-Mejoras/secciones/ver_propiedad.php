...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $id_propiedad > 0) {
    ...
    if (isset($_POST['guardar_cambios'])) {
        ...
        $stmt = $pdo->prepare(
            'UPDATE propiedades SET titulo=:titulo, tipo=:tipo, operacion=:operacion, estado=:estado,
             ...
             habitaciones=:habitaciones, banos=:banos, referencia=:referencia, descripcion=:descripcion WHERE id=:id'
        );
    ...
    if (isset($_POST['guardar_nota'])) {
        ...
        if ($nota_texto !== '') {
            $pdo->prepare('INSERT INTO notas (entity_type, entity_id, tipo, texto, usuario_id) VALUES (:et, :eid, :tipo, :texto, :uid)')
                ->execute(['et' => 'propiedad', 'eid' => $id_propiedad, 'tipo' => $nota_tipo, 'texto' => $nota_texto, 'uid' => $_SESSION['usuario']['id'] ?? null]);
        }
    ...
    if (isset($_POST['eliminar_propiedad'])) {
        try {
            ...
            $pdo->prepare("DELETE FROM notas WHERE entity_type = 'propiedad' AND entity_id = :id")->execute(['id' => $id_propiedad]);
            $pdo->prepare("DELETE FROM entidad_etiquetas WHERE entidad_tipo = 'propiedad' AND entidad_id = :id")->execute(['id' => $id_propiedad]);
            $pdo->prepare('DELETE FROM propiedades WHERE id = :id')->execute(['id' => $id_propiedad]);
            $pdo->commit();
...
$stmt = $pdo->prepare("SELECT id, tipo, texto, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i') AS fecha FROM notas WHERE entity_type = 'propiedad' AND entity_id = :id ORDER BY created_at DESC");
$stmt->execute(['id' => $id_propiedad]);
...
if (!empty($datos_propiedad['cliente_id'])) {
    $stmt = $pdo->prepare('SELECT nombre, apellido, telefono, email FROM clientes WHERE id = :id');
    $stmt->execute(['id' => $datos_propiedad['cliente_id']]);
    ...
    <details class="tag_agregar_detail">
        ...
        <div class="tag_dropdown">
            ...
            <?php foreach ($tags_disp as $t): ?>
                <form method="POST"><input type="hidden" name="etiqueta_id" value="<?php echo $t['id']; ?>"><button type="submit" name="asignar_etiqueta" class="tag_option" style="border-left:3px solid <?php echo e($t['color']); ?>"><?php echo e($t['nombre']); ?></button></form>
            <?php endforeach; ?>
                    ...
                    <li class="nota_item" id="nota-<?php echo $nota['id']; ?>">
                        ...
                        <div class="nota_edit_form" id="nota-edit-<?php echo $nota['id']; ?>" style="display:none">
                            ...
                            <div class="nota_edit_btns">
                                <form method="POST" id="fn-<?php echo $nota['id']; ?>"><input type="hidden" name="nota_id" value="<?php echo $nota['id']; ?>"><button type="submit" name="editar_nota" class="btn_guardar btn_chico">Guardar</button></form>
                                <form method="POST" data-confirm="¿Eliminar esta nota?"><input type="hidden" name="nota_id" value="<?php echo $nota['id']; ?>"><button type="submit" name="eliminar_nota" class="btn_peligro btn_chico">🗑️</button></form>
