...
if (isset($_GET['exportar']) && $_GET['exportar'] === 'csv') {
    ...
    $stmt = $pdo->prepare('SELECT * FROM ofertas WHERE usuario_id = :uid ORDER BY id DESC');
    $stmt->execute(['uid' => (int)($_SESSION['usuario']['id'] ?? 0)]);
...
$stmt = $pdo->prepare('SELECT id, titulo, referencia, precio FROM propiedades WHERE equipo = :equipo ORDER BY titulo ASC');
...
$stmt = $pdo->query('SELECT id, nombre, apellido FROM clientes ORDER BY nombre ASC');
