...
$pdo->exec("CREATE TABLE IF NOT EXISTS post_venta (
    ...
    INDEX(propiedad_id), INDEX(etapa), INDEX(usuario_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");

// Agregar columna usuario_id si la tabla ya existe pero no la tiene
try {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['crear_seguimiento'])) {
        ...
        if ($prop_id > 0) {
            $stmt = $pdo->prepare('INSERT INTO post_venta (propiedad_id, cliente_id, usuario_id, etapa, notas, fecha_venta) VALUES (:pid, :cid, :uid, :etapa, :notas, :fv)');
            $stmt->execute(['pid' => $prop_id, 'cid' => $cli_id, 'uid' => $usuario_id, 'etapa' => 'pendiente_docs', 'notas' => $notas, 'fv' => $fecha_venta]);
    ...
    if (isset($_POST['mover_etapa'])) {
        ...
        if ($pv_id > 0 && array_key_exists($nueva_etapa, $columnas)) {
            $pdo->prepare('UPDATE post_venta SET etapa = :etapa WHERE id = :id AND usuario_id = :uid')->execute(['etapa' => $nueva_etapa, 'id' => $pv_id, 'uid' => $usuario_id]);
    ...
    if (isset($_POST['eliminar_seguimiento'])) {
        $pv_id = (int) ($_POST['pv_id'] ?? 0);
        if ($pv_id > 0) {
            $pdo->prepare('DELETE FROM post_venta WHERE id = :id AND usuario_id = :uid')->execute(['id' => $pv_id, 'uid' => $usuario_id]);
...
$stmt = $pdo->query("SELECT id, titulo, precio FROM propiedades WHERE estado IN ('Vendido','Reservado') ORDER BY titulo");
$props_vendidas = $stmt->fetchAll();

$stmt = $pdo->query("SELECT id, CONCAT(nombre, ' ', apellido) AS nombre_completo FROM clientes ORDER BY nombre");
