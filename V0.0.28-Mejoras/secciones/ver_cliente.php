...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $id_cliente > 0) {
    if (isset($_POST['guardar_cambios'])) {
        ...
        $stmt = $pdo->prepare(
            'UPDATE clientes SET nombre = :nombre, apellido = :apellido, telefono = :telefono,
                ...
                zona_interesada = :zona_interesada, comentarios = :comentarios WHERE id = :id'
        );
        ...
        if ($nota_texto !== '') {
            $stmt = $pdo->prepare(
                'INSERT INTO notas (entity_type, entity_id, tipo, texto, usuario_id)
                 VALUES (:etype, :eid, :tipo, :texto, :uid)'
            );
            $stmt->execute(['etype' => 'cliente', 'eid' => $id_cliente, 'tipo' => $nota_tipo, 'texto' => $nota_texto, 'uid' => $_SESSION['usuario']['id'] ?? null]);
        }
    ...
    if (isset($_POST['eliminar_cliente'])) {
        try {
            ...
            $pdo->prepare("DELETE FROM notas WHERE entity_type = 'cliente' AND entity_id = :id")->execute(['id' => $id_cliente]);
            $pdo->prepare("DELETE FROM entidad_etiquetas WHERE entidad_tipo = 'cliente' AND entidad_id = :id")->execute(['id' => $id_cliente]);
            $pdo->prepare('DELETE FROM clientes WHERE id = :id')->execute(['id' => $id_cliente]);
            $pdo->commit();
...
$stmt = $pdo->prepare("SELECT id, tipo, texto, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i') AS fecha FROM notas WHERE entity_type = 'cliente' AND entity_id = :id ORDER BY created_at DESC");
$stmt->execute(['id' => $id_cliente]);
