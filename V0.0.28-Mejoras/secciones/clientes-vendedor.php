<?php
/*
 * Sección: Clientes vendedor — V0.0.27
...
if (isset($_GET['exportar']) && $_GET['exportar'] === 'csv') {
    $stmt = $pdo->prepare('SELECT nombre, apellido, telefono, email, operacion, created_at FROM clientes WHERE tipo = :tipo ORDER BY id DESC');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['crear_cliente'])) {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO clientes (tipo, nombre, apellido, telefono, email, operacion) VALUES (:tipo, :nombre, :apellido, :telefono, :email, :operacion)'
        );
        $stmt->execute([
...
// Paginación
$stmt_total = $pdo->prepare('SELECT COUNT(*) FROM clientes WHERE tipo = :tipo');
$stmt_total->execute(['tipo' => 'vendedor']);
