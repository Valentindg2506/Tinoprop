...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['importar_clientes']) && !empty($_FILES['csv_file']['tmp_name'])) {
        // Validación de tipo de archivo en servidor (extensión + tamaño).
        $ext = strtolower(pathinfo($_FILES['csv_file']['name'] ?? '', PATHINFO_EXTENSION));
        if ($ext !== 'csv' || $_FILES['csv_file']['error'] !== UPLOAD_ERR_OK || $_FILES['csv_file']['size'] > 5 * 1024 * 1024) {
            flash_set('error', 'El archivo debe ser un CSV válido de menos de 5 MB.');
            $mensaje_error = flash_get('error');
            goto fin_post;
        }
