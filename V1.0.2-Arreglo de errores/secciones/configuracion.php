...
// Filtra también por inmobiliaria_id para defensa en profundidad.
$stmt = $pdo->prepare('SELECT id, nombre, email, rol FROM usuarios WHERE id = :id' . sql_iid() . ' LIMIT 1');
$stmt->execute(['id' => $usuario_id] + sql_iid_params());
$usuario = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handlers de etiquetas — dentro del bloque POST para que csrf_verify() ya se haya ejecutado.
    etiquetas_asegurar_tabla($pdo);
    if (isset($_POST['crear_etiqueta_config'])) {
        $tn = trim($_POST['tag_nombre'] ?? '');
        $tc = $_POST['tag_color'] ?? '#3b82f6';
        // Validar que el color sea un valor hex CSS válido.
        if (!preg_match('/^#[0-9a-fA-F]{3,6}$/', $tc)) {
            $tc = '#3b82f6';
        }
        if ($tn) {
            $new_tag_id = etiqueta_crear($pdo, $tn, $tc);
            actividad_registrar($pdo, 'crear', 'etiqueta', (int) $new_tag_id, 'Etiqueta creada: ' . $tn);
        }
        header('Location: index.php?seccion=configuracion');
        exit;
    }
    if (isset($_POST['eliminar_etiqueta_config'])) {
        $tid = (int) ($_POST['tag_id'] ?? 0);
        if ($tid > 0) {
            etiqueta_eliminar($pdo, $tid);
            actividad_registrar($pdo, 'eliminar', 'etiqueta', $tid, 'Etiqueta eliminada');
        }
        header('Location: index.php?seccion=configuracion');
        exit;
    }
            ...
            <div class="tags_lista_config">
                ...
                <?php else: ?>
                    ...
                    <div class="tag_config_item">
                        <?php $safe_color = preg_match('/^#[0-9a-fA-F]{3,6}$/', $tag['color']) ? $tag['color'] : '#3b82f6'; ?>
                        <span class="tag_badge" style="background:<?php echo $safe_color; ?>"><?php echo e($tag['nombre']); ?></span>
