 ...
 * Rol: gestión de usuarios de una inmobiliaria.
 * Acceso: Jefe (su propia inmobiliaria) o SuperAdmin (cualquier inmobiliaria).
 */
...
// Solo el jefe de su inmobiliaria o superadmin puede gestionar
...
// Roles que puede asignar según quien gestiona
$roles_asignables = $roles;
unset($roles_asignables['superadmin']); // Nadie crea superadmins desde aquí
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['crear_usuario'])) {
        ...
        // Verificar email único dentro de la misma inmobiliaria.
        if (empty($errores)) {
            ...
            if ($check->fetch()) $errores[] = 'Ese email ya está registrado en esta inmobiliaria.';
        }
        ...
        if (!empty($errores)) {
            ...
            // Almacenar la contraseña temporal en sesión para mostrarla UNA SOLA VEZ (no en flash permanente).
    ...
    if (isset($_POST['reset_password'])) {
        ...
        if ($uid > 0) {
            ...
            if ($stmt->rowCount() > 0) {
                // Mostrar contraseña temporal una sola vez vía sesión, no en flash persistente.
...
<?php endif; ?>
<?php
// Mostrar contraseña temporal UNA SOLA VEZ y limpiarla de sesión.
