...
function obtenerDatosFavoritoDesdeEvento(nombre, eventRef) {
    // Usar el evento pasado como parámetro (evitar window.event, deprecado en algunos navegadores).
    const objetivo = (eventRef || (typeof event !== 'undefined' ? event : null))?.target;
...
function actualizar_menu() {
    ...
    if (favoritos.length === 0) {
        ...
        favoritos.forEach(fav => {
            ...
            const url = fav.url || '#';
            const icono = iconoMap[nombre] || '⭐';

            const li = document.createElement('li');

            const a = document.createElement('a');
            a.href = url;
            a.setAttribute('data-tooltip', nombre);
...
/* =============================================
   HAMBURGUESA MÓVIL — toggle sidebar en móvil
   ========================================= */
...
/* =============================================
   INIT TODOS LOS MÓDULOS V0.0.31
   ========================================= */
document.addEventListener('DOMContentLoaded', () => {
    initSidebarToggle();
    initHamburguesaMovil();
    initBuscadorGlobal();
    initNotificaciones();
    initOrdenarTablas();
