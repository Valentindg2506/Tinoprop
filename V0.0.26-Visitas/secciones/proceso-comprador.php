...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['crear_proceso'])) {
        ...
        if ($propiedad_id > 0) {
            ...
            $check->execute(['id' => $propiedad_id, 'equipo' => 'comprador']);
            if (!$check->fetch()) {
        ...
        if ($propiedad_id > 0 && empty($errores)) {
            ...
            $check->execute(['pid' => $propiedad_id, 'equipo' => 'comprador']);
            if ($check->fetch()) {
    ...
    if (isset($_POST['eliminar_proceso'])) {
        ...
        if ($id_eliminar > 0) {
            ...
            $stmt->execute(['id' => $id_eliminar, 'equipo' => 'comprador']);
        }
...
$stmt->execute(['equipo' => 'comprador', 'equipo2' => 'comprador']);
$propiedades_disponibles = $stmt->fetchAll();
?>

<div class="encabezado_seccion">
    <h2>Proceso de Propiedades - Comprador</h2>
