...
/**
 * ============================================================================
 * ARCHIVO: propiedades-comprador.php
 * SECCIÓN: Propiedades en Venta del equipo Comprador
 * ============================================================================
 *
 * ¿QUÉ HACE ESTE ARCHIVO?
 * Gestiona las propiedades inmobiliarias en VENTA asignadas al equipo
 * comprador. Permite crear, listar y eliminar propiedades.
 *
 * DIFERENCIA CON propiedades-vendedor.php:
 * La estructura es idéntica, pero filtra por equipo='comprador' en vez
 * de 'vendedor'. La idea es que cada equipo tiene su propia cartera
 * de propiedades que gestiona de forma independiente.
 *
 * FUNCIONALIDADES:
 *   - CREAR propiedades con datos completos (título, tipo, precio, etc.)
 *   - LISTAR en formato tarjetas visuales (cards)
 *   - ELIMINAR con borrado de notas asociadas (transacción SQL)
 *
 * SEGURIDAD: csrf_verify(), e(), sql_iid(), transacciones, PDO preparadas
 */
...
/* ========================================================================
   CONTROLADOR POST — Crear y eliminar propiedades
   ========================================================================
   Procesa las acciones de los formularios enviados por POST. */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    /* Protección CSRF */
    csrf_verify();
    /* ----------------------------------------------------------------
       ACCIÓN: CREAR NUEVA PROPIEDAD EN VENTA (equipo comprador)
       ---------------------------------------------------------------- */
    if (isset($_POST['crear_propiedad'])) {
        ...
        /* INSERCIÓN EN BD: la propiedad se guarda con equipo='comprador'
           y operacion='venta'. Los campos opcionales pueden ser null. */
        $stmt = $pdo->prepare(
    ...
    /* ----------------------------------------------------------------
       ...
       ----------------------------------------------------------------
       Usa transacción SQL para borrar notas asociadas + propiedad. */
...
/* ========================================================================
   CONSULTA DE PROPIEDADES EN VENTA (equipo comprador)
   ========================================================================
   Obtiene las propiedades para mostrar en las tarjetas. */
...
<!-- =====================================================================
     PARTE VISUAL (HTML) — Tarjetas de propiedades en venta (comprador)
     ===================================================================== -->
...
<!-- FORMULARIO DE ALTA de propiedad en venta -->
...
<!-- GRID DE TARJETAS: cada propiedad se muestra como un card visual -->
<div class="grid_cards">
    <?php foreach ($propiedades_venta as $propiedad): ?>
        <!-- TARJETA DE PROPIEDAD con datos, precio y acciones -->
