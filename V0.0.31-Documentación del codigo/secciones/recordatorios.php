...
/**
 * ============================================================================
 * RECORDATORIOS - CRM TINOPROP
 * ============================================================================
 *
 * ¿Qué es este archivo?
 * ---------------------
 * Permite al agente inmobiliario gestionar su agenda de recordatorios
 * con un calendario visual mensual. Es como una agenda personal
 * dentro del CRM.
 *
 * Funcionalidades:
 * - Calendario mensual interactivo: muestra badges (insignias) con
 *   la cantidad de recordatorios que hay cada día.
 ...
 * - Tipos de recordatorio: Llamada, Visita, Reunión, Nota importante,
 *   Seguimiento, Otro.
 * - Estados: pendiente, completado, cancelado.
 * - Opcionalmente se puede vincular a un prospecto/cliente por su ID.
 *
 * Arquitectura:
 * - El calendario y la lista inicial se renderizan en PHP (servidor).
 * - Las operaciones CRUD se realizan con JavaScript + fetch()
 *   llamando a la API 'api/recordatorios.php' sin recargar la página.
 * - La API responde con JSON y se usa location.reload() para
 *   actualizar la vista tras cada operación.
 *
 * Seguridad:
 * - Token CSRF enviado en la cabecera X-CSRF-Token para operaciones POST
 * - e() escapa todo el HTML para prevenir XSS
 * - Los datos del usuario se aíslan por inmobiliaria_id
 *
 * @archivo  secciones/recordatorios.php
...
/*
 * PARÁMETROS DE NAVEGACIÓN DEL CALENDARIO
 * ¿Qué mes y año mostrar? Si la URL tiene ?mes=5&ano=2026,
 * mostramos mayo 2026. Si no, mostramos el mes y año actual.
 * La fecha seleccionada es la que aparece resaltada en el calendario
 * y determina qué recordatorios se muestran en la lista.
 */
$mes_actual = isset($_GET['mes']) ? (int) $_GET['mes'] : date('n'); // date('n') = mes sin cero (1-12)
$ano_actual = isset($_GET['ano']) ? (int) $_GET['ano'] : date('Y');

/*
 * VALIDACIÓN DE LA FECHA SELECCIONADA
 * Verificamos que tenga formato YYYY-MM-DD con una expresión regular.
 * Si alguien escribe algo inválido en la URL, usamos la fecha de hoy.
 * preg_match() comprueba si el texto cumple el patrón regular.
 */
...
/*
 * CARGA DE RECORDATORIOS DEL MES
 * Obtenemos TODOS los recordatorios del mes para poder marcar en el
 * calendario cuántos hay cada día (los badges con números).
 * recordatorios_por_mes() es un helper que hace la consulta SQL.
 */
$recordatorios_mes = recordatorios_por_mes($pdo, $mes_actual, $ano_actual);

/*
 * Creamos un array asociativo donde la clave es el día del mes (1, 2, 3...)
 * y el valor es la cantidad de recordatorios de ese día.
 * Ejemplo: [5 => 2, 12 => 1, 20 => 3] = "el día 5 tiene 2 recordatorios"
 */
$recordatorios_por_dia = [];
foreach ($recordatorios_mes as $r) {
    $dia = (int) date('j', strtotime($r['fecha_recordatorio']));
 ...
 * Estos son los que se muestran en la lista principal (panel derecho).
 */
$recordatorios_hoy = recordatorios_por_fecha($pdo, $fecha_seleccionada);

/*
 * DATOS PARA CONSTRUIR EL CALENDARIO
 * - $primer_dia_mes: qué día de la semana es el 1 del mes (0=Domingo...6=Sábado)
 * - $dias_en_mes: cuántos días tiene el mes (28, 29, 30 o 31)
 * El calendario se construye como una tabla HTML de 7 columnas (Dom-Sáb)
 */
...
/* Tipos de recordatorio disponibles en el desplegable del formulario */
...
<div class="seccion_recordatorios">
    ...
    <div class="contenedor_recordatorios">
        ...
        <!-- Aquí se muestran los recordatorios del día seleccionado.
             Cada uno es una "tarjeta" con tipo, hora, descripción,
             estado y botones de editar/eliminar.
             Si no hay recordatorios, se muestra un mensaje informativo. -->
...
/*
 * ─────────────────────────────────────────────────────────────
 * JAVASCRIPT - LÓGICA DEL CLIENTE (NAVEGADOR)
 * ─────────────────────────────────────────────────────────────
 * Este JS maneja la interacción con el usuario en el navegador:
 *
 * 1. Click en día del calendario:
 *    Cambia la selección visual y recarga la página con la nueva fecha.
 *
 * 2. Botón "Nuevo":
 *    Abre el formulario de creación con los campos vacíos.
 *
 * 3. Envío del formulario (crear/actualizar):
 *    Envía los datos como JSON a la API usando fetch().
 *    La API devuelve {success: true/false} y recargamos.
 *
 * 4. Botón "Editar":
 *    Pide los datos del recordatorio a la API (GET) y los
 *    carga en el formulario para edición.
 *
 * 5. Botón "Eliminar":
 *    Muestra un confirm() y si acepta, envía la eliminación.
 *
 * Todas las operaciones de escritura envían el token CSRF
 * en la cabecera 'X-CSRF-Token' para seguridad.
 */
