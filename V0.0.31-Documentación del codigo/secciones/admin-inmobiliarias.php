...
/*
 * ============================================================================
 * SECCIÓN: GESTIÓN DE INMOBILIARIAS (Solo SuperAdmin) — TinoProp CRM
 * ============================================================================
 *
 * ¿QUÉ HACE ESTE ARCHIVO?
 * Este archivo permite al SuperAdmin gestionar las AGENCIAS INMOBILIARIAS
 * que usan el sistema. TinoProp es "multi-tenant", lo que significa que
 * VARIAS inmobiliarias comparten la misma aplicación, pero cada una solo
 * ve SUS propios datos (clientes, propiedades, etc.).
 *
 * Desde aquí el SuperAdmin puede:
 *   1. CREAR una nueva inmobiliaria (nombre, CIF, dirección, contacto, límite usuarios)
 *   2. EDITAR los datos de una inmobiliaria existente
 *   3. ACTIVAR / DESACTIVAR una inmobiliaria (sin borrarla)
 *   4. VER la lista de todas las inmobiliarias registradas
 *   5. ACCEDER a los usuarios de cada inmobiliaria (enlace a admin-usuarios)
 *
 * ¿POR QUÉ ES IMPORTANTE?
 * Cada inmobiliaria es un "inquilino" (tenant) del sistema. Al crearla,
 * se establece su límite de usuarios y se le permite empezar a trabajar.
 * Si se desactiva, sus usuarios ya no podrán acceder sin perder datos.
 *
 * SEGURIDAD:
 * - verificar_acceso('admin-inmobiliarias'): solo SuperAdmin puede entrar.
 * - csrf_verify(): protege formularios contra ataques CSRF.
 *
 * ============================================================================
 */

/* Cargamos bootstrap y verificamos permisos de acceso. */
...
/* Nos aseguramos de que la tabla de inmobiliarias exista en la BD.
   Esto es útil en la primera ejecución o tras una migración. */
...
/* ============================================================================
 * BLOQUE POST: Acciones CRUD (Crear, Leer, Actualizar, Desactivar)
 * ============================================================================
 ...
 * Aquí no eliminamos, sino que activamos/desactivamos (soft delete). */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrf_verify();

    /* ── ACCIÓN: CREAR INMOBILIARIA ──
       Se recogen los datos del formulario y se crea un nuevo registro.
       El campo 'nombre' es obligatorio; el resto son opcionales.
       max_usuarios define cuántas cuentas puede tener esa agencia. */
    if (isset($_POST['crear_inmobiliaria'])) {
        ...
        if ($nombre === '') {
            ...
            /* Llamamos a la función que inserta la nueva inmobiliaria en la BD. */
    ...
    /* ── ACCIÓN: EDITAR INMOBILIARIA ──
       Actualiza los datos de una inmobiliaria existente. Se necesita
       su ID (viene como campo oculto del formulario) y un nombre válido. */
    ...
    /* ── ACCIÓN: ACTIVAR / DESACTIVAR INMOBILIARIA ──
       En vez de borrar una inmobiliaria (lo cual eliminaría todos sus datos),
       la "desactivamos". Sus usuarios no podrán iniciar sesión, pero los
       datos se conservan. Si más adelante se quiere reactivar, se puede hacer.
       Este patrón se llama "soft delete" (borrado suave). */
...
/* Obtenemos la lista completa de inmobiliarias registradas en el sistema. */
$inmobiliarias = inmobiliarias_listar($pdo);

/* Si el admin hizo clic en "Editar" (✏️) en alguna inmobiliaria, cargamos
   sus datos para rellenar el formulario de edición.
   El parámetro viene por URL: ?editar=3 (donde 3 es el ID). */
$editando = null;
