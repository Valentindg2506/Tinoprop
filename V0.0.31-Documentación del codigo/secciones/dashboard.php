...
/**
 * ============================================================================
 * DASHBOARD DEL CRM TINOPROP
 * ============================================================================
 *
 * ¿Qué es este archivo?
 * ---------------------
 * Es la página principal que ve un agente inmobiliario al entrar al sistema.
 * Funciona como un "panel de control" o "cuadro de mandos" que muestra
 * un resumen visual de toda la actividad de la inmobiliaria.
 *
 * ¿Qué muestra?
 * - KPIs (Key Performance Indicators): números clave como clientes activos,
 *   propiedades en venta/alquiler, visitas pendientes, etc.
 * - Embudo comercial: gráfico que muestra en qué etapa están los prospectos
 *   (nuevo → contactado → cerrado → descartado).
 * - Alertas y avisos: las últimas notas de tipo "aviso" creadas.
 * - Actividad reciente: historial de las últimas acciones realizadas.
 * - Propiedades destacadas: las que más visitas han recibido.
 * - Recordatorios próximos: tareas pendientes de los próximos 7 días.
 * - Visitas de hoy: agenda de visitas programadas para el día actual.
 * - Panel supervisor: rendimiento por agente (solo visible para supervisores).
 * - Panel jefe: informe general de la empresa (solo visible para jefes).
 *
 * Filtros disponibles:
 ...
 *
 * Los filtros se guardan como preferencias del usuario para que la próxima
 * vez que entre, vea los mismos filtros que eligió la última vez.
 *
 * Seguridad:
 * - Aislamiento multi-tenant con sql_iid() (cada inmobiliaria solo ve sus datos)
 * - Filtro por usuario con sql_uid() (agentes solo ven lo suyo)
 * - Protección CSRF en el guardado de orden de widgets
 * - Escape HTML con e() en toda la salida para prevenir XSS
 *
 * @archivo  secciones/dashboard.php
...
/*
 * ─────────────────────────────────────────────────────────────
 * CONFIGURACIÓN DE FILTROS VÁLIDOS
 * ─────────────────────────────────────────────────────────────
 * Definimos qué valores se permiten en cada filtro del dashboard.
 * Esto es importante para la seguridad: si un usuario intenta
 * enviar un valor inventado por la URL, lo ignoramos y usamos
 * el valor por defecto.
 *
 * - Equipos: filtra por tipo de equipo comercial
 * - Periodos: rango de fechas para las estadísticas
 * - Operaciones: tipo de operación inmobiliaria
 */
$pdo = db(); // Obtiene la conexión a la base de datos (patrón singleton: solo se crea una vez)
...
/*
 * ─────────────────────────────────────────────────────────────
 * ENDPOINT POST: GUARDAR/RESETEAR ORDEN DE WIDGETS
 * ─────────────────────────────────────────────────────────────
 * El usuario puede arrastrar y soltar las tarjetas del dashboard
 * para reorganizarlas a su gusto. Cuando lo hace, el frontend
 * envía el nuevo orden por POST como JSON.
 *
 * Acciones posibles:
 * - 'guardar': almacena el orden actual del usuario
 * - 'reset': borra las preferencias de orden (vuelve al default)
 *
 * Este bloque responde con JSON (no HTML) porque lo llama
 * JavaScript con fetch(), no un formulario normal.
 */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dashboard_orden_accion'])) {
    csrf_verify(); // Verifica el token CSRF para evitar ataques de falsificación de solicitudes
...
/*
 * ─────────────────────────────────────────────────────────────
 * CARGA DE FILTROS: PREFERENCIAS DEL USUARIO + URL
 * ─────────────────────────────────────────────────────────────
 * Prioridad de los filtros:
 * 1. Si la URL trae un parámetro ?equipo=Vendedor, se usa ese.
 * 2. Si no, se busca la preferencia guardada del usuario.
 * 3. Si tampoco existe, se usa el valor por defecto ('Todos', etc.).
 *
 * Esto permite que el dashboard "recuerde" los filtros del usuario
 * pero también se pueda compartir un enlace con filtros específicos.
 */
...
/*
 * ─────────────────────────────────────────────────────────────
 * TRADUCCIÓN DE FILTROS DE INTERFAZ A VALORES DE BASE DE DATOS
 * ─────────────────────────────────────────────────────────────
 * En la interfaz mostramos "Vendedor" (con mayúscula y legible),
 * pero en la base de datos se almacena como "vendedor" (minúscula).
 * Aquí hacemos esa traducción. Si el filtro es "Todos", dejamos
 * la variable como null para NO aplicar filtro (se muestran todos).
 */
$equipo_db = null;
...
/*
 * Calculamos la fecha de inicio según el periodo seleccionado.
 * Si es "Mes actual", usamos el primer día del mes.
 * Si es "Últimos 90 días", retrocedemos 90 días desde hoy.
 * Si es "Año", usamos el 1 de enero del año actual.
 * Esta fecha se usa en las consultas SQL como: WHERE created_at >= $desde
 */
$desde = null;
if ($filtro_periodo === 'Mes actual') {
    $desde = date('Y-m-01'); // Ejemplo: 2026-03-01
 ...
 * ─────────────────────────────────────────────────────────────
 * En lugar de escribir muchas consultas diferentes, construimos
 * trozos de SQL que se añaden o no según los filtros activos.
 *
 * Ejemplo: si $equipo_db es 'vendedor', $condicion_equipo_clientes
 * será " AND tipo = :equipo". Si no hay filtro, será una cadena vacía "".
 *
 * sql_iid() y sql_uid() son funciones de seguridad multi-tenant:
 * - sql_iid(): añade "AND inmobiliaria_id = X" para aislar datos por inmobiliaria
 * - sql_uid(): añade "AND usuario_id = X" para que agentes solo vean sus datos
 */
...
/*
 * ─── CONSULTAS KPI ─────────────────────────────────────────
 * Cada KPI es una consulta COUNT(*) que cuenta registros.
 * Se usan "prepared statements" (consultas preparadas) de PDO
 * para prevenir inyección SQL: los valores NUNCA se concatenan
 * directamente en la consulta, sino que se pasan como parámetros.
 */

// KPI 1: ¿Cuántos clientes activos hay? (filtrados por equipo, fecha e inmobiliaria)
$stmt->execute($params);
...
/*
 * ─────────────────────────────────────────────────────────────
 * COMPARACIÓN CON PERIODO ANTERIOR (VARIACIÓN PORCENTUAL)
 * ─────────────────────────────────────────────────────────────
 * Para mostrar si las métricas suben o bajan (▲ +15% o ▼ -10%),
 * necesitamos los datos del periodo anterior equivalente.
 * Ejemplo: si el periodo es "Mes actual" (marzo 2026),
 * comparamos con febrero 2026.
 */
$desde_anterior = null;
if ($filtro_periodo === 'Mes actual') {
 ...
 * ─────────────────────────────────────────────────────────────
 * A partir de aquí se cargan los datos para los diferentes
 * paneles/widgets que se muestran debajo de los KPIs.
 */

/*
 * Panel de ALERTAS: últimos avisos importantes.
 * Se usa LEFT JOIN para conectar las notas con clientes y propiedades,
 * así podemos mostrar "Aviso sobre el cliente Juan García" o
 * "Aviso sobre la propiedad Piso en Madrid".
 * TIMESTAMPDIFF calcula cuántos minutos pasaron desde la creación
 * para mostrar "hace 5 min", "hace 2h", etc.
 */
$stmt = $pdo->prepare(
...
/*
 * Panel de PROPIEDADES DESTACADAS: las 3 propiedades con más visitas.
 * Usa subconsultas (SELECT COUNT(*) FROM visitas WHERE...) para contar
 * visitas y ofertas de cada propiedad sin hacer múltiples consultas.
 * visitas_asegurar_tabla() crea la tabla si no existe (patrón de migración ligera).
 */
visitas_asegurar_tabla($pdo);
...
/*
 * Panel de RECORDATORIOS: las próximas 5 tareas pendientes (7 días vista).
 * Solo muestra recordatorios del usuario logueado (no de otros agentes).
 * Excluye los cancelados. Se ordenan por fecha y hora ascendente.
 */
...
/*
 * ─────────────────────────────────────────────────────────────
 * PANELES EXCLUSIVOS POR ROL (ACCESO RESTRINGIDO)
 * ─────────────────────────────────────────────────────────────
 * Estos paneles SOLO se muestran si el usuario tiene un nivel
 * suficiente. tiene_nivel('supervisor') devuelve true para
 * supervisores, jefes y superadmin.
 */

/*
 * Panel SUPERVISOR: tabla con el rendimiento de cada agente.
 * Muestra cuántos clientes, propiedades, prospectos, visitas y ofertas
 * gestiona cada agente. Usa LEFT JOIN + GROUP BY para obtener todos
 * los datos en UNA sola consulta (evita el problema N+1 que haría
 * una consulta por cada agente y sería muy lento).
 */
$panel_supervisor = [];
...
/*
 * Panel JEFE: informe ejecutivo con métricas globales de la empresa.
 * - Total usuarios activos de la inmobiliaria
 * - Valor total de propiedades en cartera (disponibles)
 * - Valor total reservado (en proceso de cierre)
 * - Tasa de conversión: % de prospectos que se convirtieron en venta
 * - Mejor agente del periodo (el que más visitas realizó)
 */
$panel_jefe = [];
if (tiene_nivel('jefe')) {
