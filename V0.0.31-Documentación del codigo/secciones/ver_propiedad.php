...
/*
 * ============================================================================
 * SECCIÓN: VER PROPIEDAD (Ficha detallada) — TinoProp CRM Inmobiliario
 * ============================================================================
 *
 * ¿QUÉ HACE ESTE ARCHIVO?
 * Este archivo muestra la "ficha completa" de una propiedad inmobiliaria.
 * Es como la página de un anuncio en Idealista o Fotocasa, pero para uso
 * interno del agente inmobiliario. Desde aquí puedes:
 *
 *   1. VER todos los datos de la propiedad (título, precio, metros, etc.)
 *   2. EDITAR esos datos directamente (haciendo clic en el lápiz ✎)
 *   3. SUBIR / VER fotos de la propiedad (galería de imágenes)
 *   4. AGREGAR / EDITAR / ELIMINAR notas sobre la propiedad
 *   5. ASIGNAR etiquetas (tags) de colores para clasificarla
 *   6. VER el timeline (historial cronológico) de actividad
 *   7. CONTACTAR al propietario por WhatsApp o email
 *   8. DUPLICAR la propiedad (crear una copia)
 *   9. ELIMINAR la propiedad si ya no es necesaria
 *
 * ESTRUCTURA DEL ARCHIVO:
 *   Línea 1-130: Procesamiento de formularios (POST) — acciones del usuario
 *   Línea 130+:  Carga de datos (GET) — consultas a la BD
 *   Línea final:  HTML + PHP mezclado — la interfaz visual
 *
 * SEGURIDAD:
 * - csrf_verify() protege contra ataques CSRF.
 * - sql_iid() filtra por inmobiliaria (multi-tenant).
...
/* Cargamos el bootstrap: sesión, funciones, conexión a BD, etc. */
require_once __DIR__ . '/../inc/bootstrap.php';

/* Obtenemos la conexión a la base de datos. */
$pdo = db();

/* Leemos el ID de la propiedad desde la URL (ej: ?id=15).
   (int) fuerza conversión a entero por seguridad. */
$id_propiedad = (int) ($_GET['id'] ?? 0);

/* 'origen' indica desde qué listado vino el usuario, para el botón "Volver". */
$origen = $_GET['origen'] ?? 'propiedades-vendedor';

/* Mensajes flash: notificaciones temporales de éxito o error. */
$mensaje_error = flash_get('error');
$mensaje_exito = flash_get('success');

/* Nos aseguramos de que la tabla de imágenes exista en la BD.
   Si es la primera vez que se usa esta funcionalidad, la crea automáticamente. */
imagenes_asegurar_tabla($pdo);

/* ============================================================================
 * BLOQUE POST: Procesamiento de formularios
 * ============================================================================
 * Se ejecuta SOLO cuando el usuario envía un formulario (método POST).
 * Cada 'if' dentro maneja una acción diferente del usuario. */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $id_propiedad > 0) {
    /* Verificación CSRF: confirma que el formulario viene de nuestra web. */
    csrf_verify();

    /* ── ACCIÓN: GUARDAR CAMBIOS de la propiedad ── */
    if (isset($_POST['guardar_cambios'])) {
        $errores = [];

        /* Recogemos y limpiamos los datos del formulario.
           strtolower() convierte la operación a minúsculas para estandarizar
           ("Venta" y "venta" se tratan igual). */
        ...
        /* Listas blancas (whitelist): definimos qué valores son válidos.
           Solo aceptamos operaciones y estados que estén en estas listas.
           Esto previene que alguien envíe valores maliciosos o inventados. */
        ...
        /* Validaciones: comprobamos que los campos obligatorios están presentes
           y que los valores pertenecen a las listas válidas.
           validar_enum() comprueba que el valor esté en la lista permitida. */
        ...
        /* Si hay errores, redirigimos de vuelta mostrando los mensajes. */
        if (!empty($errores)) {
        ...
        /* Preparamos los datos para la actualización en la BD.
           Los campos numéricos opcionales (metros, habitaciones, baños) se
           convierten a null si están vacíos, porque en la BD son campos
           que permiten nulo (nullable). */
        $datos_update = [
        ...
        /* Ejecutamos el UPDATE con consulta preparada.
           sql_iid() añade el filtro de inmobiliaria (multi-tenant). */
        $stmt = $pdo->prepare(
        ...
        /* Registramos la edición en el log de actividad (aparece en el timeline). */
    ...
    /* ── ACCIÓN: GUARDAR NUEVA NOTA ──
       Notas internas sobre la propiedad (ej: "Ya tiene cédula de habitabilidad"). */
    if (isset($_POST['guardar_nota'])) {
        $nota_texto = trim($_POST['nota_nueva'] ?? '');
        $nota_tipo = $_POST['nota_tipo'] ?? 'Nota';
        if ($nota_texto !== '') {
            /* Insertamos la nota vinculada a 'propiedad' y al ID de esta propiedad. */
    ...
    /* ── ACCIÓN: ASIGNAR ETIQUETA a la propiedad ── */
    ...
    /* ── ACCIÓN: QUITAR ETIQUETA de la propiedad ── */
    if (isset($_POST['quitar_etiqueta'])) {
        $tid = (int) ($_POST['etiqueta_id'] ?? 0);
        if ($tid > 0) entidad_quitar_etiqueta($pdo, 'propiedad', $id_propiedad, $tid);
    }

    /* ── ACCIÓN: CREAR ETIQUETA RÁPIDA y asignarla en un solo paso ── */
    ...
    /* ── ACCIÓN: DUPLICAR PROPIEDAD ──
       Crea una copia exacta de esta propiedad. Útil cuando tienes varias
       propiedades similares (ej: pisos en el mismo edificio).
       El nuevo registro recibe un ID diferente y se redirige a su ficha. */
    if (isset($_POST['duplicar_propiedad'])) {
        ...
        if ($nuevo_id) {
            ...
            /* Redirigimos a la ficha de la NUEVA propiedad duplicada. */
    ...
    /* ── ACCIÓN: ELIMINAR PROPIEDAD ──
       Operación destructiva: borra la propiedad y todos sus datos relacionados.
       Usa una transacción para que si algo falla, no quede nada a medias. */
    if (isset($_POST['eliminar_propiedad'])) {
        try {
            $pdo->beginTransaction();
            /* Paso 1: Borrar notas vinculadas a esta propiedad */
    ...
    /* Redirección POST-Redirect-GET para evitar reenvíos al refrescar. */
    header('Location: index.php?seccion=ver_propiedad&id=' . $id_propiedad . '&origen=' . urlencode($origen));
    exit;
}

/* ============================================================================
 * BLOQUE GET: Carga de datos de la propiedad para mostrar en pantalla
 * ============================================================================ */

/* Consulta principal: obtenemos todos los campos de la propiedad. */
...
/* Si no se encontró la propiedad, creamos un array vacío con las claves
   necesarias para evitar errores PHP en la interfaz. */
if (!$datos_propiedad) {
...
/* Cargamos las notas de la propiedad, de más reciente a más antigua. */
...
/* Cargamos las imágenes de la propiedad y la imagen principal (la destacada). */
$imagenes_propiedad = imagenes_obtener_propiedad($pdo, $id_propiedad);
$imagen_principal = imagen_obtener_principal($pdo, $id_propiedad);

/* Cargamos etiquetas asignadas y todas las disponibles. */
$tags_propiedad = entidad_obtener_etiquetas($pdo, 'propiedad', $id_propiedad);
$todas_tags = etiquetas_listar($pdo);

/* Timeline: historial cronológico de la propiedad. */
$timeline = timeline_entidad($pdo, 'propiedad', $id_propiedad);

/* Datos del propietario: si la propiedad tiene un cliente_id asignado,
   cargamos sus datos de contacto para mostrar botones de WhatsApp y email. */
$propietario = null;
...
/* Limpiamos el teléfono del propietario para enlaces tel: y wa.me. */
...
function subirImagenes(archivos) {
    ...
    fetch('api/imagenes.php?action=subir&_csrf_token=' + encodeURIComponent(csrfToken), { method: 'POST', body: fd })
    .then(r => r.json())
