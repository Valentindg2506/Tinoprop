...
/**
 * ========================================================================================
 * ARCHIVO: visitas-comprador.php
 * SECCIÓN: Gestión de Visitas del Equipo Comprador
 * ========================================================================================
 *
 * PROPÓSITO:
 * Este archivo gestiona las visitas a propiedades del equipo comprador dentro del CRM
 * inmobiliario TinoProp. Funciona exactamente igual que visitas-vendedor.php pero
 * filtrado para el equipo comprador (propiedades que se buscan para compradores).
 *
 * FUNCIONALIDAD PRINCIPAL:
 * - Crear nuevas visitas asociadas a propiedades y/o clientes del equipo comprador.
 * - Editar datos de visitas existentes.
 * - Cambiar el estado rápidamente desde la tabla (pendiente/realizada/cancelada).
 * - Eliminar visitas (también borra el recordatorio del calendario asociado).
 * - Exportar visitas del comprador a CSV.
 *
 * DIFERENCIA CON visitas-vendedor.php:
 * Solo cambia el filtro de equipo ('comprador' en vez de 'vendedor').
 * El equipo comprador gestiona propiedades que los clientes quieren comprar,
 * mientras que el equipo vendedor gestiona propiedades que se están vendiendo.
 *
 * SEGURIDAD:
 * - csrf_verify(), e(), sql_iid() para protección CSRF, XSS y multi-tenant.
 *
 * @version V0.0.31
 */

/* Cargamos bootstrap: inicializa sesión, BD, funciones auxiliares y autenticación. */
...
/* ── EXPORTACIÓN CSV ──────────────────────────────────────────────────────────
   Genera un archivo CSV descargable con todas las visitas del comprador.
   Se activa cuando el usuario pulsa el botón "Exportar CSV" en la tabla. */
if (isset($_GET['exportar']) && $_GET['exportar'] === 'csv') {
    /* Aseguramos que la tabla 'visitas' existe antes de consultarla. */
    visitas_asegurar_tabla($pdo);
    $uid = (int)($_SESSION['usuario']['id'] ?? 0);

    /* Consulta SQL: obtiene visitas del comprador con datos de propiedad y cliente.
       LEFT JOIN permite obtener visitas incluso si no tienen propiedad/cliente asignado. */
    $stmt = $pdo->prepare('SELECT v.*, p.titulo AS propiedad, c.nombre AS cliente_nombre, c.apellido AS cliente_apellido FROM visitas v LEFT JOIN propiedades p ON v.propiedad_id = p.id LEFT JOIN clientes c ON v.cliente_id = c.id WHERE v.usuario_id = :uid AND v.equipo = "comprador"' . sql_iid('v') . ' ORDER BY v.id DESC');
    $stmt->execute(['uid' => $uid] + sql_iid_params());
    exportar_csv($stmt->fetchAll(PDO::FETCH_ASSOC), 'visitas_comprador.csv');
}

/* Identificador de esta sección, usado para redirecciones y navegación. */
$origen = 'visitas-comprador';

/* Mensajes flash: notificaciones de éxito/error de la acción anterior. */
$mensaje_error = flash_get('error');
$mensaje_exito = flash_get('success');

/* Estados válidos para una visita. Se validan en el servidor para
   asegurar que nadie envíe un estado inválido manipulando el HTML. */
$estados_visita = ['pendiente', 'realizada', 'cancelada'];

/* ═════════════════════════════════════════════════════════════════════════════
   CONTROLADOR POST — Procesa formularios (crear, editar, eliminar visitas).
   Se ejecuta SOLO cuando el navegador envía datos por POST.
   ═════════════════════════════════════════════════════════════════════════════ */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    /* Protección CSRF: verifica que el formulario se envió desde nuestra web. */
    csrf_verify();

    /* ── CREAR NUEVA VISITA (equipo comprador) ──────────────────────────── */
    if (isset($_POST['crear_visita'])) {
        ...
        /* Creamos la visita para el equipo 'comprador'.
           El último parámetro true = crear también recordatorio en calendario. */
        $id = visita_crear($pdo, 'comprador', $propiedad_id ?: null, $cliente_id ?: null, $fecha, $hora, $observaciones, true);

        if ($id) {
            actividad_registrar($pdo, 'crear', 'visita', $id, 'Visita comprador creada');
    ...
    if (isset($_POST['editar_visita'])) {
        ...
        if ($id_editar > 0) {
            /* Recogemos los campos editables. */
            $propiedad_id = (int) ($_POST['propiedad_id'] ?? 0);
            $cliente_id = (int) ($_POST['cliente_id'] ?? 0);
            $fecha = trim($_POST['fecha_visita'] ?? '');
            $hora = trim($_POST['hora_visita'] ?? '') ?: null;
            ...
            /* Validamos fecha y que el estado sea uno de los permitidos. */
    ...
    /* ── CAMBIAR ESTADO RÁPIDO (desde el desplegable de la tabla) ───── */
    if (isset($_POST['cambiar_estado'])) {
        $id_cambiar = (int) ($_POST['id'] ?? 0);
        $nuevo_estado = trim($_POST['estado'] ?? '');
        /* Solo actualizamos si el ID es válido y el estado está en la lista permitida. */
    ...
    /* ── ELIMINAR VISITA (y su recordatorio asociado en el calendario) ── */
    if (isset($_POST['eliminar_visita'])) {
        $id_eliminar = (int) ($_POST['id'] ?? 0);
        if ($id_eliminar > 0) {
            /* El parámetro true indica que también se borrará el recordatorio. */
            $ok = visita_eliminar($pdo, $id_eliminar, true);
            if ($ok) {
                /* Registramos la eliminación en el log de actividad. */
                actividad_registrar($pdo, 'eliminar', 'visita', $id_eliminar, 'Visita comprador eliminada');
            }
            flash_set($ok ? 'success' : 'error', $ok ? 'Visita eliminada correctamente.' : 'Error al eliminar.');
        }
    }

    /* Redirección POST-Redirect-GET para evitar reenvío de formularios al refrescar. */
...
/* Filtro de estado desde la URL. Por defecto muestra todas. */
$filtro = $_GET['estado'] ?? 'todos';
if (!in_array($filtro, ['todos', 'pendiente', 'realizada', 'cancelada'])) {
    $filtro = 'todos';
}

/* Obtenemos las visitas del equipo comprador aplicando el filtro de estado. */
$visitas = visitas_listar($pdo, 'comprador', $filtro);

/* Cargamos propiedades del equipo comprador para los desplegables del formulario.
   sql_iid() añade la condición de inmobiliaria para aislamiento multi-tenant. */
...
/* Cargamos clientes de tipo comprador para el formulario. */
...
<!-- ======================================================================
     VISTA HTML — Interfaz del usuario para gestionar visitas del comprador.
     ====================================================================== -->

<!-- Botón para crear nueva visita (hace scroll al formulario) -->
