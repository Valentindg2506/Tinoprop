...
/*
 * =====================================================================
 *  SISTEMA DE RECORDATORIOS (Agenda / To-Do list)
 * =====================================================================
 *
 * Los recordatorios funcionan como una agenda personal para cada
 * agente. Pueden crear avisos con fecha, hora, tipo y descripción
 * para no olvidarse de tareas importantes:
 *   - "Llamar a cliente García el viernes a las 10:00"
 *   - "Preparar documentación para firma el lunes"
 *   - "Visita programada al piso de la calle Gran Vía"
 *
 * Cada recordatorio tiene un estado: "pendiente", "completado"
 * o "cancelado". También puede estar vinculado a un prospecto
 * o a una visita.
 *
 * IMPORTANTE: Los recordatorios están vinculados al usuario
 * (no a la inmobiliaria), así que cada agente ve SOLO sus
 * propios recordatorios. Un jefe no ve los de sus empleados
 * (es información personal de organización).
 * =====================================================================
 */

/*
 * recordatorios_asegurar_tabla() — CREAR TABLA SI NO EXISTE
 *
 * Crea la tabla de recordatorios con todos sus campos:
 *   - id: Identificador único del recordatorio
 *   - usuario_id: A quién pertenece este recordatorio
 *   - tipo: Categoría libre ("Llamada", "Visita", "Reunión", etc.)
 *   - descripcion: Texto libre con el detalle del recordatorio
 *   - fecha_recordatorio: Día en que debe saltar el aviso
 *   - hora_recordatorio: Hora exacta (opcional)
 *   - prospecto_id: Vinculación opcional con un prospecto
 *   - estado: "pendiente" / "completado" / "cancelado"
 *   - inmobiliaria_id: Para filtrado multi-tenant
 *
 * Los índices aceleran las consultas más frecuentes:
 *   - idx_usuario_id: Buscar por usuario ("mis recordatorios")
 *   - idx_fecha: Buscar por fecha ("recordatorios de hoy")
 *   - idx_estado: Filtrar por estado ("solo pendientes")
 */
 ...
 *
 * Inserta un recordatorio en la base de datos para el usuario actual.
 * El usuario se obtiene automáticamente de la sesión (no se pasa
 * como parámetro, por comodidad y seguridad).
 *
 * Parámetros:
 *   $tipo: Categoría del recordatorio ("Llamada", "Reunión", etc.)
 *   $descripcion: Texto libre con el detalle
 *   $fecha: Fecha en formato YYYY-MM-DD (ej: "2026-03-15")
 *   $hora: Hora en formato HH:MM (opcional, puede ser null)
 *   $prospecto_id: ID del prospecto vinculado (opcional)
 *
 * Devuelve: El ID del recordatorio creado, o null si no hay sesión.
 */
...
/*
 * recordatorios_por_fecha() — OBTENER RECORDATORIOS DE UN DÍA
 *
 * Devuelve todos los recordatorios del usuario actual para una
 * fecha concreta, ordenados por hora. Se usa en la vista de
 * calendario cuando el usuario hace clic en un día.
 *
 * Ejemplo:
 *   $hoy = recordatorios_por_fecha($pdo, '2026-03-02');
 *   // Devuelve un array con cada recordatorio del 2 de marzo
 */
...
/*
 * recordatorios_por_mes() — OBTENER RECORDATORIOS DE UN MES COMPLETO
 *
 * Devuelve todos los recordatorios del mes indicado para el usuario
 * actual. Se usa para pintar el calendario mensual: cada día que
 * tenga recordatorios se marca con un indicador visual.
 *
 * Parámetros:
 *   $mes: Número del mes (1 = enero, 12 = diciembre)
 *   $ano: Año con 4 dígitos (ej: 2026)
 *
 * YEAR() y MONTH() son funciones SQL que extraen el año y mes
 * de una fecha. Permitiendo buscar todos los recordatorios cuya
 * fecha_recordatorio caiga dentro del mes indicado.
 */
...
/*
 * recordatorio_obtener() — OBTENER UN RECORDATORIO POR SU ID
 *
 * Busca un recordatorio específico verificando que pertenezca
 * al usuario actual (seguridad: un usuario no puede ver los
 * recordatorios de otro usuario a través de la URL).
 *
 * Devuelve un array con todos los datos, o null si no existe
...
/*
 * recordatorio_actualizar() — MODIFICAR UN RECORDATORIO EXISTENTE
 *
 * Actualiza todos los campos editables de un recordatorio.
 * Solo funciona si el recordatorio pertenece al usuario actual
 * (el WHERE incluye usuario_id para garantizar la seguridad).
 *
 * Devuelve true si se modificó algo, false si el recordatorio
 * no existe, no pertenece al usuario, o los datos eran iguales.
 */
...
/*
 * recordatorio_eliminar() — BORRAR UN RECORDATORIO
 *
 * Elimina permanentemente un recordatorio de la base de datos.
 * Solo funciona si pertenece al usuario actual (seguridad).
 *
 * Devuelve true si se eliminó, false si no existía o no era
 * del usuario.
 *
 * NOTA: A diferencia de inmobiliarias/usuarios que se "desactivan"
 * en vez de borrar, los recordatorios SÍ se eliminan al 100%
 * porque son datos temporales de organización personal.
 */
...
/*
 * =====================================================================
 *  SISTEMA DE IMÁGENES DE PROPIEDADES
 * =====================================================================
 *
 * Cada propiedad puede tener múltiples fotos (galería). Las imágenes
 * se almacenan en el servidor en storage/uploads/propiedades/ y su
 * metadatos se guardan en la tabla imagenes_propiedades.
 *
 * Una imagen puede marcarse como "principal" para usarla como
 * miniatura en listados. Si ninguna está marcada, se usa la primera.
 *
 * Funciones disponibles:
 *   - imagenes_asegurar_tabla(): Crea la tabla si no existe
 *   - imagen_subir(): Sube un archivo y guarda referencia en BD
 *   - imagenes_obtener_propiedad(): Lista todas las fotos de una propiedad
 *   - imagen_obtener_principal(): Obtiene la foto principal/primera
 *   - imagen_eliminar(): Borra foto de BD y disco
 *   - imagen_marcar_principal(): Marca una foto como la principal
 * =====================================================================
 */

/*
 * imagenes_asegurar_tabla() — CREAR TABLA SI NO EXISTE
 *
 * Crea la tabla que almacena los metadatos de cada imagen:
 *   - propiedad_id: A qué propiedad pertenece (con FOREIGN KEY)
 *   - nombre_archivo: Nombre generado único en el servidor
 *   - nombre_original: Nombre original que tenía el archivo del usuario
 *   - ruta_archivo: Ruta relativa para acceder vía web
 *   - es_principal: true/false para la foto destacada
 *
 * La FOREIGN KEY con ON DELETE CASCADE significa que si se borra
 * una propiedad, automáticamente se borran sus imágenes de la BD
 * (pero los archivos físicos deben borrarse aparte).
 */
...
/*
 * imagen_subir() — SUBIR UNA IMAGEN AL SERVIDOR
 *
 * Proceso de subida segura de una imagen:
 *
 *   1. VALIDACIÓN DE TIPO: Comprueba el MIME real del archivo
 *      (no confía en la extensión que envía el cliente, porque
 *      un atacante podría renombrar un virus.exe a virus.jpg).
 *      Solo acepta: JPEG, PNG y WebP.
 *
 *   2. VALIDACIÓN DE TAMAÑO: Máximo 10 MB por imagen.
 *
 *   3. DIRECTORIO: Crea storage/uploads/propiedades/ si no existe.
 *
 *   4. NOMBRE ÚNICO: Genera un nombre como "img_42_65a3b2c1.jpg"
 *      usando uniqid() para evitar colisiones de nombres.
 *      La extensión se determina por el MIME real, no por el
 *      nombre original del archivo.
 *
 *   5. MOVER ARCHIVO: move_uploaded_file() mueve el archivo
 *      desde la carpeta temporal de PHP a su destino final.
 *
 *   6. GUARDAR EN BD: Registra los metadatos de la imagen.
 *
 * Devuelve: El ID de la imagen creada, o null si falló.
 */
...
/*
 * imagenes_obtener_propiedad() — LISTAR TODAS LAS IMÁGENES
 *
 * Devuelve todas las imágenes de una propiedad, con la principal
 * first (ORDER BY es_principal DESC). Incluye id, ruta, nombre
 * original y si es principal.
 */
...
/*
 * imagen_obtener_principal() — OBTENER LA FOTO DESTACADA
 *
 * Devuelve la imagen marcada como principal de una propiedad.
 * Si ninguna está marcada, devuelve la primera subida.
 * Se usa en listados para mostrar una miniatura representativa.
 */
...
/*
 * imagen_eliminar() — BORRAR UNA IMAGEN (BD + ARCHIVO FÍSICO)
 *
 * Elimina una imagen de la base de datos Y del disco duro.
 * Antes de borrar, verifica que la imagen pertenezca a una
 * propiedad de la inmobiliaria del usuario (seguridad multi-tenant).
 *
 * Proceso:
 *   1. Consulta la ruta del archivo verificando que la propiedad
 *      pertenezca a la inmobiliaria del usuario (INNER JOIN + sql_iid).
 *   2. Si no la encuentra (no existe o es de otra inmobiliaria),
 *      devuelve false sin tocar nada.
 *   3. Borra el archivo físico del disco con unlink().
 *   4. Borra el registro de la base de datos con DELETE.
 *
...
/*
 * imagen_marcar_principal() — ESTABLECER FOTO PRINCIPAL
 *
 * Marca una imagen como la foto principal/destacada de su propiedad.
 * Solo puede haber UNA principal por propiedad, así que:
 *   1. Primero desmarca TODAS las imágenes de esa propiedad
 *   2. Luego marca la seleccionada como principal
 *
 * Incluye verificación de seguridad multi-tenant (INNER JOIN
 * con propiedades + sql_iid) para que solo puedas cambiar
 * imágenes de propiedades de tu propia inmobiliaria.
 */
...
/*
 * =====================================================================
 *  PROPIEDADES SCRAPEADAS (Datos importados de portales inmobiliarios)
 * =====================================================================
 *
 * Esta tabla almacena propiedades extraídas automáticamente de portales
 * inmobiliarios externos (idealista, fotocasa, etc.) mediante un
 * proceso de web scraping.
 *
 * El campo raw_hash (SHA-256 del contenido original) permite detectar
 * si una propiedad ya existe o ha cambiado desde la última importación.
 * La restricción UNIQUE en url evita duplicados.
 *
 * Los try/catch de ALTER TABLE al final son migraciones que añaden
 * columnas nuevas (imagen_url, imagenes_json, ascensor) a tablas
 * ya existentes de versiones anteriores.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  SISTEMA DE VISITAS (Agenda de visitas a propiedades)
 * =====================================================================
 *
 * Las visitas son citas programadas entre un agente y un cliente
 * para ver una propiedad. Están organizadas por "equipo":
 *   - equipo: "vendedor" → visitas del lado captador (dueño quiere vender)
 *   - equipo: "comprador" → visitas del lado comprador (busca vivienda)
 *
 * SINCRONIZACIÓN CON RECORDATORIOS:
 *   Las visitas están vinculadas con los recordatorios. Cuando se crea
 *   una visita, automáticamente se crea un recordatorio asociado.
 *   Si el estado de la visita cambia (realizada, cancelada), el
 *   recordatorio también se actualiza. Esta sincronización bidireccional
 *   evita que el agente tenga que gestionar ambos manualmente.
 *
 * Funciones disponibles:
 *   - visitas_asegurar_tabla(): Crea la tabla
 *   - visita_crear(): Nueva visita + recordatorio automático
 *   - visita_crear_desde_recordatorio(): Crear visita a partir de recordatorio
 *   - visitas_listar(): Lista con datos de propiedad y cliente
 *   - visita_actualizar_estado(): Cambia estado y sincroniza recordatorio
 *   - visita_actualizar(): Modifica todos los campos
 *   - visita_eliminar(): Borra visita y su recordatorio vinculado
 * =====================================================================
 */

/*
 * visitas_asegurar_tabla() — CREAR TABLA DE VISITAS
 *
 * Campos principales:
 *   - equipo: "vendedor" o "comprador" (divide las visitas)
 *   - propiedad_id: Qué propiedad se visita (opcional)
 *   - cliente_id: Quién hace la visita (opcional)
 *   - fecha_visita / hora_visita: Cuándo
 *   - estado: "pendiente" / "realizada" / "cancelada"
 *   - observaciones: Notas de texto libre
 *   - recordatorio_id: Vínculo con la tabla de recordatorios
 *   - usuario_id: Agente responsable de esta visita
 *   - inmobiliaria_id: Para filtrado multi-tenant
 */
...
/*
 * visita_crear() — CREAR UNA NUEVA VISITA
 *
 * Registra una visita y opcionalmente crea un recordatorio vinculado.
 * La sincronización automática recordatorio-visita evita trabajo manual.
 *
 * Parámetros:
 *   $equipo: "vendedor" o "comprador"
 *   $propiedad_id: ID de la propiedad a visitar (puede ser null)
 *   $cliente_id: ID del cliente que visita (puede ser null)
 *   $fecha, $hora: Cuándo se programa la visita
 *   $observaciones: Notas adicionales
 *   $crear_recordatorio: Si es true (por defecto), crea un
 *     recordatorio de tipo "Visita" automáticamente
 *
 * Devuelve: ID de la visita creada, o null si no hay sesión.
 */
...
/*
 * visita_crear_desde_recordatorio() — SINCRONIZACIÓN INVERSA
 *
 * Cuando un agente crea un recordatorio de tipo "Visita" en su
 * calendario, esta función crea automáticamente la visita correspondiente
 * en el sistema. Es la sincronización inversa: recordatorio → visita.
 *
 * Verifica que no exista ya una visita vinculada al mismo recordatorio
 * para evitar duplicados.
 *
 * Devuelve: ID de la visita creada, o null si ya existía o hubo error.
 */
...
/*
 * visitas_listar() — OBTENER LISTA DE VISITAS CON DATOS RELACIONADOS
 *
 * Devuelve las visitas de un equipo (vendedor/comprador) del usuario
 * actual, enriquecidas con datos de la propiedad y el cliente
 * mediante LEFT JOIN (unión de tablas).
 *
 * LEFT JOIN significa: "trae los datos de la otra tabla si existen,
 * pero si no existen, pon null". Así una visita sin propiedad
 * asignada sigue apareciendo en la lista.
 *
 * El filtro sql_iid('v') asegura aislamiento multi-tenant.
 * Se puede filtrar por estado (pendiente/realizada/cancelada/todos).
 */
...
/*
 * visita_actualizar_estado() — CAMBIAR ESTADO Y SINCRONIZAR RECORDATORIO
 *
 * Cambia el estado de una visita (pendiente → realizada/cancelada)
 * y automáticamente actualiza el recordatorio vinculado:
 *   - Visita "realizada" → Recordatorio "completado"
 *   - Visita "cancelada" → Recordatorio "cancelado"
 *   - Visita "pendiente" → Recordatorio "pendiente"
 *
 * Esta sincronización bidireccional mantiene ambos sistemas coherentes.
 */
...
/*
 * visita_actualizar() — MODIFICAR TODOS LOS CAMPOS DE UNA VISITA
 *
 * Actualiza propiedad, cliente, fecha, hora, estado y observaciones
 * de una visita existente. Incluye:
 *   - Verificación de propiedad por usuario_id y sql_iid()
 *   - Sincronización con el recordatorio vinculado (actualiza
 *     descripción, fecha, hora y estado del recordatorio)
 *
 * Devuelve true si se modificó algo, false si no.
 */
...
/*
 * visita_eliminar() — BORRAR UNA VISITA Y SU RECORDATORIO
 *
 * Elimina una visita de la base de datos. Por defecto, también
 * elimina el recordatorio vinculado ($eliminar_recordatorio = true).
 *
 * Proceso:
 *   1. Obtiene el recordatorio_id vinculado (si existe)
 *   2. Elimina la visita
 *   3. Elimina el recordatorio (si se encontró y $eliminar_recordatorio)
 *
 * La verificación por usuario_id + sql_iid() impide que un usuario
 * borre visitas de otro agente o de otra inmobiliaria.
 */
 ...
 * =====================================================================
 *
 * Las ofertas representan las propuestas económicas que recibe una
 * propiedad. Un comprador interesado ofrece un precio, y el vendedor
 * puede aceptar, rechazar o hacer una contraoferta.
 *
 * Estados de una oferta:
 *   - pendiente: Recién creada, esperando respuesta
 *   - aceptada: El vendedor aceptó el precio
 *   - rechazada: El vendedor no acepta
 *   - contraoferta: El vendedor propone un precio diferente
 *
 * Cuando el estado es "contraoferta", se usa el campo
 * contraoferta_importe para almacenar el precio alternativo.
 *
 * Funciones:
 *   - ofertas_asegurar_tabla(): Crea la tabla
 *   - oferta_crear(): Nueva oferta con estado "pendiente"
 *   - ofertas_listar(): Lista con datos de propiedad y cliente
 *   - oferta_cambiar_estado(): Acepta/rechaza/contraoferta
 *   - oferta_actualizar(): Edita todos los campos
 *   - oferta_eliminar(): Borra una oferta
 * =====================================================================
 */

/*
 * ofertas_asegurar_tabla() — CREAR TABLA DE OFERTAS
 *
 * Campos principales:
 *   - propiedad_id: Sobre qué propiedad es la oferta
 *   - cliente_id: Quién hace la oferta (puede ser null)
 *   - nombre_ofertante: Nombre libre si no es cliente registrado
 *   - importe: Cantidad ofrecida en euros
 *   - fecha_oferta: Cuándo se hizo la oferta
 *   - estado: pendiente/aceptada/rechazada/contraoferta
 *   - contraoferta_importe: Precio alternativo si hay contraoferta
 *   - notas: Observaciones adicionales
 *   - usuario_id + inmobiliaria_id: Seguridad multi-tenant
 *
 * Los try/catch al final son migraciones para añadir inmobiliaria_id
 * y su índice en bases de datos de versiones anteriores.
 */
...
/*
 * oferta_crear() — REGISTRAR UNA NUEVA OFERTA
 *
 * Crea una oferta con estado "pendiente" para una propiedad.
 * El ofertante puede ser un cliente registrado (cliente_id)
 * o una persona externa (nombre_ofertante como texto libre).
 *
 * Devuelve: ID de la oferta creada, o null si no hay sesión.
 */
...
/*
 * ofertas_listar() — LISTA DE OFERTAS CON DATOS DE PROPIEDAD Y CLIENTE
 *
 * Obtiene las ofertas del usuario actual con información enriquecida
 * mediante LEFT JOINs con propiedades y clientes.
 * Se puede filtrar por estado o ver todas (estado = 'todos').
...
/*
 * oferta_cambiar_estado() — ACEPTAR, RECHAZAR O CONTRAOFERTAR
 *
 * Cambia el estado de una oferta. Si el nuevo estado es
 * "contraoferta" y se proporciona un importe alternativo,
 * también lo guarda en contraoferta_importe.
 *
 * Verificación: solo el usuario dueño de la oferta y de la
 * misma inmobiliaria puede cambiar su estado.
 */
...
/*
 * oferta_actualizar() — EDITAR TODOS LOS CAMPOS DE UNA OFERTA
 *
 * Permite modificar la propiedad, cliente, importe, fecha, estado,
 * contraoferta y notas de una oferta existente.
 * Solo el usuario propietario de la inmobiliaria correcta puede editar.
 */
...
/*
 * oferta_eliminar() — BORRAR UNA OFERTA
 *
 * Elimina permanentemente una oferta de la base de datos.
 * Solo funciona para ofertas del usuario actual y su inmobiliaria.
 */
 ...
 *   Imagina que estás logueado en TinoProp y abres otra pestaña
 *   del navegador. Un sitio malicioso en esa otra pestaña podría
 *   enviar un formulario a TinoProp a tus espaldas (por ejemplo,
 *   "borrar todos mis clientes"). Como estás logueado, el servidor
 *   lo aceptaría pensando que lo hiciste tú.
 *
 * ¿Cómo se previene?
 *   Con un "token" (código secreto) que solo conoce TU sesión.
 *   Cada formulario incluye este código oculto. Cuando se envía,
 *   el servidor comprueba que el código del formulario coincide
 *   con el de la sesión. Un sitio externo no puede conocer este
 *   código, así que sus formularios falsos serán rechazados.
 *
 * Funciones:
 *   csrf_token():       Genera/obtiene el token de la sesión actual
 *   csrf_field():       Genera el HTML del campo oculto del formulario
 *   csrf_verify():      Valida el token en peticiones POST normales
 *   csrf_verify_api():  Valida el token en peticiones API (AJAX/JSON)
 *
 * bin2hex(random_bytes(32)): Genera 32 bytes aleatorios criptográficamente
 *   seguros y los convierte en texto hexadecimal (64 caracteres).
 *   Es prácticamente IMPOSIBLE de adivinar.
 *
 * hash_equals(): Compara dos strings de forma segura contra ataques
 *   de timing (mide cuánto tarda la comparación para deducir caracteres).
 * =====================================================================
 */
...
/*
 * csrf_verify_api() — VERIFICAR TOKEN CSRF EN PETICIONES API
 *
 * Igual que csrf_verify() pero para peticiones AJAX/API.
 * Busca el token en tres lugares (en orden de prioridad):
 *   1. Header HTTP "X-CSRF-Token" (el más común en AJAX)
 *   2. Campo JSON en el cuerpo de la petición
 *   3. Parámetro POST o GET clásico
 *
 * Si el token no coincide, devuelve error 403 en formato JSON
...
/*
 * =====================================================================
 *  HISTORIAL DE ACTIVIDAD (Registro de auditoría)
 * =====================================================================
 *
 * Todas las acciones importantes se registran en la tabla
 * actividad_log: quién hizo qué, cuándo y desde qué IP.
 * Esto sirve para:
 *   - Auditoría: saber quién borró un cliente
 *   - Seguridad: detectar accesos sospechosos
 *   - Historial: ver la evolución de una entidad
 *
 * Cada registro tiene:
 *   - usuario_id/nombre: Quién realizó la acción
 *   - accion: Tipo de acción (crear, editar, eliminar, login...)
 *   - entidad/entidad_id: Qué se afectó ("cliente" #42)
 *   - descripcion: Texto legible ("Creó cliente Juan García")
 *   - datos_extra: Datos adicionales en JSON (valores anteriores, etc.)
 *   - ip: Dirección IP desde donde se hizo
 *   - inmobiliaria_id: Para filtrado multi-tenant
 *
 * actividad_registrar() es la función más usada: se llama desde
 * TODOS los módulos que crean/editan/eliminan datos.
 *
 * actividad_listar() y actividad_contar() se usan en la página
 * de "Historial" para mostrar el registro paginado.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  NOTIFICACIONES INTELIGENTES (Alertas automáticas del sidebar)
 * =====================================================================
 *
 * Genera alertas automáticas analizando el estado de los datos.
 * Se muestran como badges (números) en el icono de campana del
 * sidebar. NO se almacenan en BD: se calculan en CADA carga de página.
 *
 * Alertas que genera:
 *   - Visitas de hoy pendientes (tipo: info)
 *   - Ofertas pendientes hace +5 días (tipo: aviso)
 *   - Prospectos sin contactar hace +2 semanas (tipo: aviso)
 *   - Recordatorios para hoy (tipo: info)
 *   - Recordatorios atrasados (tipo: peligro)
 *
 * Cada alerta es un array con: tipo, icono, texto y enlace.
 * Los try/catch previenen que un error en una consulta rompa
 * todas las demás notificaciones.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  CACHÉ DE CONSULTAS EN SESIÓN
 * =====================================================================
 *
 * Sistema sencillo de caché temporal que almacena resultados de
 * consultas costosas en $_SESSION para no repetirlas en la misma
 * carga de página o en cargas sucesivas cercanas.
 *
 * Funciones:
 *   cache_get($clave, $ttl): Lee un valor cacheado. Si ha pasado más
 *     de $ttl segundos (por defecto 60) desde que se guardó, lo
 *     invalida y devuelve null (dato caducado).
 *
 *   cache_set($clave, $valor): Guarda un valor con timestamp actual.
 *
 *   cache_flush(): Borra toda la caché (se usa tras ediciones
 *     para forzar datos frescos).
 *
 * NOTA: Esta caché es por USUARIO (está en $_SESSION), no global.
 *   No sustituye a sistemas de caché como Redis/Memcached.
 * =====================================================================
 */
 ...
 * =====================================================================
 *
 * Permite descargar datos en formato CSV (archivo de texto con
 * columnas separadas por punto y coma ";") que se puede abrir
 * directamente en Excel, Google Sheets o LibreOffice Calc.
 *
 * ¿Cómo funciona internamente?
 *   1. Envía cabeceras HTTP para que el navegador lo descargue
 *      como archivo en vez de mostrarlo en pantalla.
 *   2. Escribe el BOM UTF-8 (3 bytes mágicos) para que Excel
 *      reconozca correctamente los acentos y la ñ.
 *   3. Escribe la primera fila con los nombres de las columnas
 *      (obtenidos de array_keys del primer registro).
 *   4. Escribe cada fila de datos.
 *   5. Cierra el archivo y termina (exit).
 *
 * fopen('php://output') es un "archivo virtual" que escribe
 * directamente en la respuesta HTTP del navegador.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  BREADCRUMBS (Migas de pan / Ruta de navegación)
 * =====================================================================
 *
 * Los breadcrumbs muestran dónde está el usuario dentro de la app:
 *   Inicio > Vendedor > Propiedades
 *
 * Son como las migas de pan de Hansel y Gretel: permiten saber
 * el camino recorrido y volver atrás fácilmente.
 *
 * generar_breadcrumbs(): Dada una sección, devuelve un array con
 *   las "migas" y sus enlaces. El mapa define la jerarquía de
 *   cada sección.
 *
 * renderizar_breadcrumbs(): Convierte el array en HTML con
 *   enlaces clicables. El último elemento (página actual) NO
 *   tiene enlace y lleva aria-current="page" para accesibilidad.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  PAGINACIÓN (Dividir listas largas en páginas)
 * =====================================================================
 *
 * Cuando hay cientos de clientes o propiedades, no se muestran
 * todos a la vez (sería lentísimo). Se dividen en páginas de
 * N elementos (por defecto 15).
 *
 * paginar(): Calcula todos los datos necesarios:
 *   - total: Cuántos registros hay en total
 *   - por_pagina: Cuántos se muestran por página
 *   - pagina_actual: En qué página estamos
 *   - total_paginas: Cuántas páginas hay
 *   - offset: Desde qué registro empezar (para SQL LIMIT/OFFSET)
 *   - tiene_anterior/tiene_siguiente: Si hay botón de navegar
 *
 * renderizar_paginacion(): Genera los botones HTML:
 *   ← Anterior | 1 | 2 | 3 | ... | 10 | Siguiente →
 *   Con clase CSS activa en la página actual y puntos suspensivos
 *   (...) para no mostrar 100 botones cuando hay muchas páginas.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  BÚSQUEDA GLOBAL (Buscador del sidebar)
 * =====================================================================
 *
 * Busca simultáneamente en clientes, propiedades y prospectos.
 * Muestra resultados instantáneos mientras el usuario escribe.
 *
 * Usa SQL LIKE '%término%' que busca el texto en cualquier parte
 * del campo (inicio, medio o final). Es case-insensitive.
 *
 * Para cada tipo de entidad busca en los campos más relevantes:
 *   - Clientes: nombre, apellido, email, teléfono
 *   - Propiedades: título, ubicación, referencia, dirección
 *   - Prospectos: nombre, teléfono
 *
 * Todos los filtros incluyen sql_iid() para respetar el
 * aislamiento multi-tenant.
 *
 * Devuelve un array de resultados con: tipo, icono, título,
 * detalle y URL para navegar al registro.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  NOTAS — Editar y eliminar comentarios sobre entidades
 * =====================================================================
 *
 * Las notas son comentarios de texto libre adjuntos a clientes,
 * propiedades o prospectos. Un agente puede escribir "Cliente
 * interesado en zona norte" y adjuntarlo a la ficha del cliente.
 *
 * nota_actualizar(): Modifica texto y tipo de una nota existente.
 * nota_eliminar(): Borra una nota permanentemente.
 *
 * Ambas incluyen sql_iid_pair() para garantizar que solo se
 * puedan tocar notas de la propia inmobiliaria.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  SISTEMA DE ETIQUETAS/TAGS (Organización por colores)
 * =====================================================================
 *
 * Las etiquetas son marcadores con nombre y color que el usuario
 * puede crear y asignar a cualquier entidad (cliente, propiedad, etc.).
 * Son como post-its de colores: "VIP" (rojo), "Urgente" (naranja),
 * "Zona Centro" (azul).
 *
 * Dos tablas trabajan juntas:
 *   - etiquetas: Define las etiquetas disponibles (nombre + color)
 *     Cada usuario tiene sus propias etiquetas (son personales).
 *   - entidad_etiquetas: Relación muchos-a-muchos entre etiquetas
 *     y entidades. Por ejemplo, el cliente #42 puede tener las
 *     etiquetas "VIP" y "Zona Centro" simultáneamente.
 *
 * La restricción UNIQUE en entidad_etiquetas evita asignar la
 * misma etiqueta dos veces al mismo cliente.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  DUPLICAR PROPIEDAD (Clonar una ficha de propiedad)
 * =====================================================================
 *
 * Crea una copia exacta de una propiedad existente con pequeñas
 * diferencias:
 *   - Título: se añade " (copia)" al final
 *   - Referencia: se añade "-COPIA"
 *   - Estado: se resetea a "Disponible"
 *   - Fechas: se regeneran automáticamente
 *
 * Útil cuando hay propiedades similares (ej: varios pisos del mismo
 * edificio) y se quiere crear uno basado en otro para ahorrar tiempo.
 *
 * Incluye sql_iid() para verificar que la propiedad original
 * pertenece a la inmobiliaria del usuario.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  IMPORTACIÓN MASIVA DESDE CSV (Carga de datos por archivo)
 * =====================================================================
 *
 * Permite cargar cientos de clientes o propiedades de golpe desde
 * un archivo CSV (tipo hoja de cálculo). El archivo debe tener:
 *   - Primera fila: nombres de columnas (cabeceras)
 *   - Separador: punto y coma (;)
 *   - Codificación: UTF-8 (con o sin BOM)
 *
 * importar_csv_clientes(): Importa clientes. Columnas obligatorias:
 *   "nombre" y "apellido". Opcionales: telefono, email, operacion,
 *   direccion, zona_interesada, presupuesto, comentarios.
 *
 * importar_csv_propiedades(): Importa propiedades. Columna obligatoria:
 *   "titulo". Opcionales: tipo, ubicacion, direccion, metros,
 *   habitaciones, banos, precio, moneda, operacion, estado, etc.
 *
 * Ambas funciones devuelven un array con:
 *   - importados: Número de registros importados con éxito
 *   - errores: Errores generales (archivo no se puede abrir, etc.)
 *   - lineas_error: Números de línea que fallaron al importar
 *
 * Cada registro importado se asigna automáticamente a la inmobiliaria
 * y usuario actuales mediante sql_iid() y sql_uid().
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  FILTROS GUARDADOS (Guardar búsquedas favoritas)
 * =====================================================================
 *
 * Permite al usuario guardar configuraciones de filtros para no
 * tener que reconfigurarlos cada vez. Por ejemplo: "Pisos en
 * zona centro < 200.000€" como filtro guardado con nombre.
 *
 * Los parámetros del filtro se almacenan como JSON (un formato
 * de texto estructurado que permite guardar múltiples valores
 * en un solo campo de la base de datos).
 *
 * filtro_guardar(): Guarda un nuevo filtro con nombre.
 * filtros_listar(): Lista los filtros del usuario para una sección.
 * filtro_eliminar(): Borra un filtro guardado.
 * =====================================================================
 */
 ...
 * =====================================================================
 *
 * El matching es una de las funciones más útiles del CRM: cruza
 * automáticamente los datos de compradores con las propiedades
 * disponibles para encontrar posibles coincidencias.
 *
 * Algoritmo:
 *   1. Obtiene los últimos 50 compradores con zona de interés
 *   2. Para cada comprador, busca propiedades que coincidan en:
 *      - Zona: La ubicación o dirección de la propiedad contiene
 *        la zona_interesada del comprador (búsqueda por texto)
 *      - Precio: El precio de la propiedad no supera el presupuesto
 *        del comprador en más del 15% (tolerancia configurable)
 *   3. Ordena los resultados poniendo primero los compradores
 *      con más coincidencias
 *
 * Ejemplo de resultado:
 *   Juan García (presupuesto: 200.000€, zona: Centro)
 *    → Piso en Gran Vía - 185.000€  (coincide zona + precio)
 *    → Ático en Sol - 210.000€     (coincide zona, precio +5%)
 *
 * Los resultados se filtran por inmobiliaria (sql_iid).
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  TIMELINE DE ACTIVIDAD POR ENTIDAD
 * =====================================================================
 *
 * Construye un historial cronológico completo de todo lo que ha
 * ocurrido con un cliente o propiedad específico. Reúne información
 * de múltiples tablas (notas, actividad_log, visitas, ofertas)
 * y las presenta ordenadas por fecha.
 *
 * Se usa en las fichas detalladas de cliente y propiedad para
 * ver la "historia" completa de la relación comercial:
 *
 *   Hoy - Visita realizada al piso de Gran Vía
 *   Ayer - Nota: "Interesado, volverá con su pareja"
 *   3 mar - Oferta: 185.000€ (pendiente)
 *   28 feb - Actividad: Cliente creado por Agente Pedro
 *
 * Cada fuente de datos se consulta por separado y luego se
 * fusionan y ordenan cronológicamente con usort().
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  PROCESO DE PROPIEDADES (Pipeline/Kanban de gestión)
 * =====================================================================
 *
 * El "proceso" es un pipeline visual (tipo Kanban) que muestra en
 * qué etapa se encuentra cada propiedad dentro del flujo de trabajo:
 *
 * Captación → Valoración → Publicación → Visitas → Oferta → Firma → Entrega
 *
 * Cada propiedad tiene una ficha en este proceso con:
 *   - etapa actual: En qué paso del pipeline está
 *   - equipo: "vendedor" o "comprador"
 *   - notas: Observaciones sobre esa etapa
 *   - usuario_id + inmobiliaria_id: Seguridad y filtrado
 *
 * Las migraciones try/catch añaden usuario_id e inmobiliaria_id
 * a bases de datos de versiones anteriores que no tenían
...
/*
 * =====================================================================
 *  PETICIONES / TICKETS (Comunicación Jefe → SuperAdmin)
 * =====================================================================
 *
 * Las peticiones son un sistema de tickets de soporte interno.
 * Permite a los jefes de cada inmobiliaria comunicarse con el
 * administrador global para reportar problemas, solicitar mejoras
 * o hacer consultas.
 *
 * Cada petición tiene:
 *   - tipo: error / mejora / consulta / urgente
 *   - prioridad: baja / media / alta / critica
 *   - estado: abierta / en_progreso / resuelta / cerrada
 *   - asunto + descripcion: Detalle del ticket
 *   - respuesta_admin: Respuesta del SuperAdmin
 *   - seccion_afectada: Dónde ocurre el problema
 *
 * Funciones:
 *   peticion_crear(): Nuevo ticket
 *   peticiones_listar_todas(): Vista SuperAdmin (todos los tickets)
 *   peticiones_listar_por_inmobiliaria(): Vista Jefe (solo sus tickets)
 *   peticion_obtener(): Detalle de un ticket
 *   peticion_responder(): SuperAdmin responde con texto y cambia estado
 *   peticion_cambiar_estado(): Cambio rápido de estado
 *   peticiones_contar_por_estado(): Contadores para dashboard
 *
 * La ordenación usa FIELD() para priorizar: crítica > alta > media > baja.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  ESTADÍSTICAS DEL SISTEMA (Panel SuperAdmin)
 * =====================================================================
 *
 * Recopila métricas de todo el sistema para el dashboard del
 * SuperAdmin. Incluye:
 *
 *   - Conteo de inmobiliarias (total y activas)
 *   - Conteo de usuarios (total, activos, con password temporal)
 *   - Usuarios agrupados por rol
 *   - Totales de entidades principales (clientes, propiedades, etc.)
 *   - Actividad de los últimos 7 días
 *   - Tamaño de la base de datos en MB
 *   - Lista de tablas con su tamaño y número de filas
 *   - Información del servidor: versión PHP, MySQL, Apache, etc.
 *   - Contadores de peticiones/tickets pendientes
 *
 * Los try/catch en cada bloque garantizan que si una tabla no
 * existe, no rompe todo el panel sino que muestra 0.
 * =====================================================================
 */
...
/*
 * =====================================================================
 *  SISTEMA LEGAL Y ACEPTACIÓN DE TÉRMINOS (RGPD/LOPDGDD)
 * =====================================================================
 *
 * La constante TERMINOS_VERSION (definida arriba) marca la versión
 * vigente de los Términos y Condiciones. Cuando se actualicen los
 * términos legales, basta con cambiar esta versión para que TODOS
 * los usuarios deban aceptarlos de nuevo (el sistema les redirige
 * a la pantalla de aceptación en bootstrap.php).
 *
 * usuario_ha_aceptado_terminos(): Compara la versión que el usuario
 *   aceptó con la versión vigente. Si son iguales, está al día.
 *
 * usuario_aceptar_terminos(): Registra la aceptación en la BD
 *   guardando la versión actual como referencia.
 *
 * requiere_aceptacion_terminos(): Indica si la página actual
 *   necesita que el usuario tenga los términos aceptados
 *   (excluye login, logout, etc.).
 * =====================================================================
 */
 ...
 *   Un hacker intenta adivinar la contraseña de un usuario probando
 *   miles de combinaciones automáticamente: "password", "123456",
 *   "qwerty", "admin", etc. Sin protección, podría probar millones.
 *
 * ¿Cómo lo prevenimos?
 *   El "rate limiting" (límite de velocidad) cuenta los intentos
 *   fallidos de login. Si una IP o email supera el límite (5 intentos
 *   en 15 minutos), se bloquea temporalmente durante 15 minutos.
 *
 * Constantes (definidas arriba):
 *   LOGIN_MAX_INTENTOS = 5     (máximo de fallos)
 *   LOGIN_VENTANA_SEGUNDOS = 900 (ventana de 15 minutos)
 *   LOGIN_BLOQUEO_SEGUNDOS = 900 (bloqueo de 15 minutos)
 *
 * Funciones:
 *   login_intentos_asegurar_tabla(): Crea tabla login_intentos
 *   login_obtener_ip(): Obtiene IP real (incluso detrás de proxy)
 *   login_intentos_bloqueado(): ¿Está bloqueado? Devuelve 0 o segundos
 *   login_intentos_registrar(): Anota un intento fallido
 *   login_intentos_limpiar(): Borra intentos tras login exitoso
 *   login_intentos_purgar(): Limpia registros de +24h
 * =====================================================================
 */

/**
 * Crea la tabla login_intentos si no existe.
 * Almacena cada intento fallido con IP, email y fecha.
 */
...
function login_intentos_limpiar(PDO $pdo, string $ip, string $email): void
{
    ...
    $stmt = $pdo->prepare('DELETE FROM login_intentos WHERE ip = :ip OR email = :email');
    $stmt->execute(['ip' => $ip, 'email' => $email]);
}
