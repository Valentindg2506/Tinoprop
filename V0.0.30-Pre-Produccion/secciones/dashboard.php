...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM clientes WHERE 1=1' . $condicion_equipo_clientes . $condicion_fecha . $condicion_iid);
$params = [] + $iid_params;
if ($equipo_db) {
...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM prospectos WHERE 1=1' . $condicion_equipo_clientes . $condicion_fecha . $condicion_iid);
$stmt->execute($params);
$prospectos_total = (int) $stmt->fetchColumn();

$stmt = $pdo->prepare(
    'SELECT COUNT(*) FROM propiedades WHERE operacion = :operacion' . $condicion_equipo_prop . $condicion_fecha . $condicion_iid
);

// KPI 3/4: propiedades de venta y alquiler bajo filtros.
$params_prop = ['operacion' => 'venta'] + $iid_params;
if ($equipo_db) {
...
$sql_res = 'SELECT COUNT(*) FROM propiedades WHERE estado = "Reservado"' . $condicion_equipo_prop . $condicion_iid;
$params_res = [] + $iid_params;
...
if ($desde_anterior) {
    ...
    $st = $pdo->prepare('SELECT COUNT(*) FROM clientes WHERE 1=1' . $condicion_equipo_clientes . $cond_prev . $condicion_iid);
    $st->execute($p_ant); $prev_clientes = (int) $st->fetchColumn();

    $st = $pdo->prepare('SELECT COUNT(*) FROM prospectos WHERE 1=1' . $condicion_equipo_clientes . $cond_prev . $condicion_iid);
    $st->execute($p_ant); $prev_prospectos = (int) $st->fetchColumn();

    $p_ant2 = $p_ant; $p_ant2['operacion'] = 'venta';
    $st = $pdo->prepare('SELECT COUNT(*) FROM propiedades WHERE operacion = :operacion' . $condicion_equipo_prop . $cond_prev . $condicion_iid);
...
$stmt = $pdo->prepare(
    'SELECT estado, COUNT(*) AS total FROM prospectos WHERE 1=1' . $condicion_equipo_clientes . $condicion_fecha . $condicion_iid . ' GROUP BY estado'
);
$stmt->execute($params);
...
$query_destacadas = 'SELECT p.id, p.titulo, p.operacion, p.equipo, p.precio, p.moneda, p.periodo, p.estado,
    ...
    FROM propiedades p WHERE 1=1' . sql_iid('p');
$params_destacadas = [] + $iid_params;
if ($operacion_db) {
...
$sql_prop_chart = 'SELECT estado, COUNT(*) AS total FROM propiedades WHERE 1=1' . $condicion_iid;
$p_prop = [] + $iid_params;
