<?php
/*
 * Sección: Proceso Propiedades (Vendedor)
 * Rol: tablero kanban para gestionar el pipeline de proceso de propiedades del equipo vendedor.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['mover_proceso_drag'])) {
        ...
        $stmt = $pdo->prepare('UPDATE proceso_propiedades SET etapa = :etapa WHERE id = :id AND equipo = :equipo');
        $stmt->execute([
            'etapa' => $etapa,
            'id'    => $id_mover,
            'equipo' => 'vendedor',
    ...
    if (isset($_POST['crear_proceso'])) {
        ...
        // Verificar que la propiedad existe y es del equipo vendedor
        if ($propiedad_id > 0) {
            $check = $pdo->prepare('SELECT id FROM propiedades WHERE id = :id AND equipo = :equipo');
            $check->execute(['id' => $propiedad_id, 'equipo' => 'vendedor']);
            if (!$check->fetch()) {
        ...
        if ($propiedad_id > 0 && empty($errores)) {
            $check = $pdo->prepare('SELECT id FROM proceso_propiedades WHERE propiedad_id = :pid AND equipo = :equipo');
            $check->execute(['pid' => $propiedad_id, 'equipo' => 'vendedor']);
            if ($check->fetch()) {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO proceso_propiedades (propiedad_id, equipo, etapa, notas)
             VALUES (:propiedad_id, :equipo, :etapa, :notas)'
        );
        $stmt->execute([
    ...
    if (isset($_POST['editar_proceso'])) {
        ...
        if ($id_editar > 0) {
            ...
            $stmt = $pdo->prepare(
                'UPDATE proceso_propiedades SET etapa = :etapa, notas = :notas WHERE id = :id AND equipo = :equipo'
            );
            $stmt->execute([
                'etapa'  => $etapa,
                'notas'  => $notas,
    ...
    if (isset($_POST['eliminar_proceso'])) {
        ...
        if ($id_eliminar > 0) {
            $stmt = $pdo->prepare('DELETE FROM proceso_propiedades WHERE id = :id AND equipo = :equipo');
            $stmt->execute(['id' => $id_eliminar, 'equipo' => 'vendedor']);
        }
...
$stmt->execute(['equipo' => 'vendedor', 'equipo2' => 'vendedor']);
$propiedades_disponibles = $stmt->fetchAll();
?>

<div class="encabezado_seccion">
    <h2>Proceso de Propiedades - Vendedor</h2>
