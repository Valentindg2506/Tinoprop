...
$salud_score = 100;
$salud_alertas = [];
if ($pet_conteo['abierta'] > 5) {
    $salud_score -= 15;
    $salud_alertas[] = $pet_conteo['abierta'] . ' peticiones abiertas sin atender';
}
if ($pet_conteo['abierta'] > 0 && $pet_conteo['abierta'] <= 5) {
    $salud_score -= 5;
}
if ($stats['usuarios_pwd_temporal'] > 0) {
    $salud_score -= 10;
    $salud_alertas[] = $stats['usuarios_pwd_temporal'] . ' usuario(s) con contraseña temporal pendiente';
}
if ($stats['db_size_mb'] > 500) {
    $salud_score -= 10;
    $salud_alertas[] = 'Base de datos grande (' . $stats['db_size_mb'] . ' MB)';
}
